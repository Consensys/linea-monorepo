...
Sync pending transactions from a source node or local transactions file to a target node. Supports both Besu and Geth
clients.

#### How Does It Work

When a source node is supplied, the tool grabs pending transactions in the txpool for both the source and target node.
Then, it compares the pending transactions in both pools to determine which transactions need to be synced, as some
transactions are already persisted in the target node. The comparison is performed on the transaction hash.

When a local transactions file is supplied, the tool reads pending transaction objects from the file and sends them
directly to the target node. File input must be a JSON array of txpool transaction objects. Each transaction must include
signature fields (`r`/`s` plus `v` or `yParity`) so the CLI can reconstruct and verify the raw transaction before
broadcasting it.

To perform the actual sync, the tool will batch send transactions in groups of 10 (controlled by the `--concurrency`
flag) to help with performance. The batch transactions are sent asynchronously, but the controlling concurrent loop is
synchronous.
...
To sync transactions from a local JSON file, pass `--file` instead of `--source`.
`synctx --file ./pending-transactions.json --target http://localhost:8501`

`--source` and `--file` are mutually exclusive, and one of them must be specified.
...
```
USAGE
  $ synctx -t <value> [-s <value>] [-f <value>] [--dry-run] [--local] [-c <value>]

CONFIG FLAGS
  -c, --concurrency=<value>  [default: 10] number of concurrent batch requests
  -f, --file=<value>         local txs file to read from, mutually exclusive with source flag
  --dry-run                  enable dry run
  --local                    enable local run, provide only forwarded ports

NODE FLAGS
  -s, --source=<value>  source node to sync from, mutually exclusive with file flag
...
EXAMPLES
  ...
  $ synctx  --file=./pending-transactions.json --target=http://geth-validator-1:8545 --concurrency=10
```
