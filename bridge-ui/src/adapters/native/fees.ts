...
export async function estimateERC20BridgingGasUsed({
  ...
  const destinationChainPublicClient = getPublicClient(wagmiConfig, { chainId: toChain.id });
  if (isUndefined(destinationChainPublicClient)) return 0n;

  const originChainPublicClient = getPublicClient(wagmiConfig, { chainId: fromChain.id });
