...
type MerkleProofCircuit struct {
	...
	Leafs  []poseidon2.GnarkOctuplet `gnark:",public"`
	Root   poseidon2.GnarkOctuplet
}
...
func TestMerkleProofGnark(t *testing.T) {
	...
	witness.Leafs = make([]poseidon2.GnarkOctuplet, nbProofs)
	for i := 0; i < nbProofs; i++ {
		witness.Proofs[i].Path = proofs[i].Path
		witness.Proofs[i].Siblings = make([]poseidon2.GnarkOctuplet, len(proofs[i].Siblings))
	...
	circuit.Leafs = make([]poseidon2.GnarkOctuplet, nbProofs)
	for i := 0; i < nbProofs; i++ {
		circuit.Proofs[i].Siblings = make([]poseidon2.GnarkOctuplet, len(proofs[i].Siblings))
	}
