...
import net.consensys.linea.nativecompressor.NativeLibUtil.copyResourceToTmpDir
import net.consensys.linea.nativecompressor.NativeLibUtil.loadJnaLibFromResource
import java.nio.file.Path

interface GoNativeBlobCompressor : Library {
...
class GoNativeBlobCompressorFactory {
  companion object {
    const val LIB_NAME = "libblob_compressor_native_jna"
    const val DICTIONARY_NAME = "compressor_dict.bin"
    private val dictionaryPath = copyDictionaryFromResources()

    @Volatile
    private var instance: GoNativeBlobCompressor? = null

    private fun copyDictionaryFromResources(): Path {
      return copyResourceToTmpDir(DICTIONARY_NAME)
    }

    /**
     * Creates a new instance of GoNativeBlobCompressor on every call.
     * At runtime, coordinator is not expected to create multiple instances of GoNativeBlobCompressor
     * Used for testing in parallel
     */
    internal fun newInstance(): GoNativeBlobCompressor {
      // return loadJnaLibFromResource(LIB_NAME, GoNativeBlobCompressor::class.java)
      return getInstance()
    }

    fun getInstance(): GoNativeBlobCompressor {
      if (instance == null) {
        synchronized(this) {
          if (instance == null) {
            instance = loadJnaLibFromResource(LIB_NAME, GoNativeBlobCompressor::class.java)
          }
        }
      }
      return instance!!
