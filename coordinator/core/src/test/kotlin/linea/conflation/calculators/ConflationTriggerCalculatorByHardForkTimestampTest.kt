...
class ConflationTriggerCalculatorByHardForkTimestampTest {
  ...
  fun `should require non-empty timestamps in constructor`() {
    assertThatThrownBy {
      ConflationTriggerCalculatorByHardForkTimestamp(emptyList(), baseTimestamp)
  ...
  fun `should require unique timestamps in constructor`() {
    ...
    assertThatThrownBy {
      ConflationTriggerCalculatorByHardForkTimestamp(duplicatedTimestamps, baseTimestamp)
    }.isInstanceOf(IllegalArgumentException::class.java)
  }
  ...
  fun `should require sorted timestamps in constructor`() {
    ...
    assertThatThrownBy {
      ConflationTriggerCalculatorByHardForkTimestamp(unsortedTimestamps, baseTimestamp)
    }.isInstanceOf(IllegalArgumentException::class.java)
  }
  ...
  fun `should accept sorted timestamps in constructor`() {
    ...
    // Should not throw
    val calculator = ConflationTriggerCalculatorByHardForkTimestamp(sortedTimestamps, baseTimestamp)
    assertThat(calculator.id).isEqualTo("TIMESTAMP_HARD_FORK")
  }
  ...
  fun `should not trigger when initialized with timestamp after hard fork`() {
    ...
    val calculator = ConflationTriggerCalculatorByHardForkTimestamp(
