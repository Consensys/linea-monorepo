 ...
 */
class ConflationTriggerCalculatorByHardForkTimestamp(
  private val hardForkTimestamps: List<Instant>,
  private val initialTimestamp: Instant,
  private val log: Logger = LogManager.getLogger(ConflationTriggerCalculatorByHardForkTimestamp::class.java),
  ...
  init {
    require(hardForkTimestamps.isNotEmpty()) {
      "Instantiation of ${this::class.simpleName} is pointless with empty timestamp list"
    }
  ...
  override fun checkOverflow(blockCounters: BlockCounters): ConflationTriggerCalculator.OverflowTrigger? {
    ...
    if (applicableTimestamp != null) {
      ...
      return ConflationTriggerCalculator.OverflowTrigger(
