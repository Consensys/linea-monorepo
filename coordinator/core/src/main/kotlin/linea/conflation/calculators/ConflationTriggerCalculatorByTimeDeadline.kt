...
class DeadlineConflationTriggerCalculatorRunner(
  private val conflationDeadlineCheckInterval: Duration,
  private val delegate: ConflationTriggerCalculatorByTimeDeadline,
) : DeferredConflationTriggerCalculator by delegate, LongRunningService {
...
class ConflationTriggerCalculatorByTimeDeadline(
  private val config: Config,
  private var lastBlockNumber: ULong,
  private val clock: Clock = Clock.System,
  private val latestBlockProvider: SafeBlockProvider,
  val log: Logger = LogManager.getLogger(ConflationTriggerCalculatorByTimeDeadline::class.java),
) : DeferredConflationTriggerCalculator {
  data class Config(
  ...
  override fun checkOverflow(blockCounters: BlockCounters): ConflationTriggerCalculator.OverflowTrigger? = null
