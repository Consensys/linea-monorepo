...
class ConflationTriggerCalculatorByDataCompressed(
  private val blobCompressor: BlobCompressor,
  private val log: Logger = LogManager.getLogger(ConflationTriggerCalculatorByDataCompressed::class.java),
  ...
  override fun checkOverflow(blockCounters: BlockCounters): ConflationTriggerCalculator.OverflowTrigger? {
    ...
    val overflowResult =
      if (blobCompressor.canAppendBlock(blockCounters.blockRLPEncoded)) {
        ...
        ConflationTriggerCalculator.OverflowTrigger(ConflationTrigger.DATA_LIMIT, blockOverSized)
      }
    log.trace(
