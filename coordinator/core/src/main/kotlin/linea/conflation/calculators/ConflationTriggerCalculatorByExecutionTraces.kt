...
class ConflationTriggerCalculatorByExecutionTraces(
  ...
  private val log: Logger = LogManager.getLogger(ConflationTriggerCalculatorByExecutionTraces::class.java),
  ...
  override fun checkOverflow(blockCounters: BlockCounters): ConflationTriggerCalculator.OverflowTrigger? {
    return if (canAppendTraces(blockCounters.tracesCounters)) {
      ...
      if (tracesValidationResult is Err) {
        ...
        ConflationTriggerCalculator.OverflowTrigger(ConflationTrigger.TRACES_LIMIT, true)
      } else {
        countOverflownTraces(blockCounters.tracesCounters)
        ConflationTriggerCalculator.OverflowTrigger(ConflationTrigger.TRACES_LIMIT, false)
      }
    }
  }
