 ...
 * 2. Request invalidity proofs when FTX fails (inclusionResult != Included)
 * 3. Track invalidity proof generation status
 * 4. Retry proof requests on failure with exponential backoff
 *
 * This component bridges the gap between FTX processing and invalidity proof generation,
 * ensuring that failed FTX get their invalidity proofs requested and tracked.
 */
class ForcedTransactionsConflationCoordinator(
  ...
  log: Logger = LogManager.getLogger(ForcedTransactionsConflationCoordinator::class.java),
