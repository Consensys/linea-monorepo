...
class GasPriceCapProviderForDataSubmission(
  ...
  init {
    require(config.maxPriorityFeePerGasCap > 0uL) {
      "maxPriorityFeePerGasCap must be greater than 0. Value=${config.maxPriorityFeePerGasCap}"
    }

    require(config.maxFeePerGasCap > 0uL) {
      "maxFeePerGasCap must be greater than 0. Value=${config.maxFeePerGasCap}"
    }

    require(config.maxFeePerBlobGasCap > 0uL) {
      "maxFeePerBlobGasCap must be greater than 0. Value=${config.maxFeePerBlobGasCap}"
    }
