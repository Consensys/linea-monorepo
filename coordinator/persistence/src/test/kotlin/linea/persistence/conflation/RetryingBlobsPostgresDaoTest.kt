...
class RetryingBlobsPostgresDaoTest {
  ...
  private val delegateBlobsDao = Mockito.mock<BlobsPostgresDao>()
  ...
  private val blobCompressionProof = BlobCompressionProof(
    compressedData = Random.Default.nextBytes(32).setFirstByteToZero(),
    conflationOrder = BlockIntervals(41U, listOf(60U)),
    prevShnarf = Random.Default.nextBytes(32),
    parentStateRootHash = Random.Default.nextBytes(32).setFirstByteToZero(),
    finalStateRootHash = Random.Default.nextBytes(32).setFirstByteToZero(),
    parentDataHash = Random.Default.nextBytes(32).setFirstByteToZero(),
    dataHash = Random.Default.nextBytes(32).setFirstByteToZero(),
    snarkHash = Random.Default.nextBytes(32),
    expectedX = Random.Default.nextBytes(32),
    expectedY = Random.Default.nextBytes(32),
    expectedShnarf = Random.Default.nextBytes(32).setFirstByteToZero(),
  ...
  fun `retrying batches dao should delegate all queries to standard dao`() {
    ...
    verify(delegateBlobsDao, Mockito.times(1)).saveNewBlob(eq(blobRecord))

    retryingBatchesPostgresDao.getConsecutiveBlobsFromBlockNumber(0U, now)
    verify(delegateBlobsDao, Mockito.times(1)).getConsecutiveBlobsFromBlockNumber(any(), eq(now))

    retryingBatchesPostgresDao.findBlobByStartBlockNumber(0U)
    verify(delegateBlobsDao, Mockito.times(1)).findBlobByStartBlockNumber(any())

    retryingBatchesPostgresDao.findBlobByEndBlockNumber(10U)
    verify(delegateBlobsDao, Mockito.times(1)).findBlobByEndBlockNumber(any())

    retryingBatchesPostgresDao.deleteBlobsUpToEndBlockNumber(10U)
    verify(delegateBlobsDao, Mockito.times(1)).deleteBlobsUpToEndBlockNumber(any())

    retryingBatchesPostgresDao.deleteBlobsAfterBlockNumber(0U)
    verify(delegateBlobsDao, Mockito.times(1)).deleteBlobsAfterBlockNumber(any())
  }
}
