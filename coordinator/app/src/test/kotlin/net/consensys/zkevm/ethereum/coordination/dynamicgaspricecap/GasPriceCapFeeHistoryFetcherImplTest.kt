  ...
  private fun createMockedWeb3jBlobExtended(): Web3jBlobExtended {
    ...
    whenever(
      ...
      .thenAnswer {
        ...
        feeHistory.reward = (1000 until 1000 + maxBlockCount.toLong())
          .map { reward -> (1..rewardPercentiles.size).map { reward.toString() } }
        feeHistory.baseFeePerGas = (10000 until 10000 + maxBlockCount.toLong() + 1)
          .map { it.toString() }
        feeHistory.gasUsedRatio = (10 until 10 + maxBlockCount.toLong())
          .map { it / 100.0 }
        feeHistory.baseFeePerBlobGas = (10000 until 10000 + maxBlockCount.toLong() + 1)
          .map { it.toString() }
        feeHistory.blobGasUsedRatio = (10 until 10 + maxBlockCount.toLong())
          .map { it / 100.0 }
        feeHistory.oldestBlock = "0x16"
