...
describe("L1ClaimingService", () => {
  ...
  let lineaRollup: LineaRollupClient;
  let l2MessageService: L2MessageServiceClient;
  ...
  beforeEach(() => {
    ...
    const { lineaRollupClient, lineaRollupLogClient, l2MessageServiceLogClient } = generateLineaRollupClient(
      l1Provider,
      l2Provider,
      TEST_CONTRACT_ADDRESS_1,
      TEST_CONTRACT_ADDRESS_2,
    ...
    lineaRollup = lineaRollupClient;
    l1LogClient = lineaRollupLogClient;
    l1ClientL2MessageServiceLogClient = l2MessageServiceLogClient;

    l2MessageService = generateL2MessageServiceClient(l2Provider, TEST_CONTRACT_ADDRESS_2, "read-only");
    l2Client2MessageServiceLogClient = new EthersL2MessageServiceLogClient(l2Provider, TEST_CONTRACT_ADDRESS_2);
  ...
  describe("getMessageProof", () => {
    ...
    // TODO: fix this test
    it.skip("should return the message proof", async () => {
      jest
        .spyOn(l1ClientL2MessageServiceLogClient, "getMessageSentEventsByMessageHash")
        .mockResolvedValueOnce([testMessageSentEvent])
        .mockResolvedValueOnce([
      ...
      const l2MerkleRoots = ["0x27ff822675b111601ab06a174f8c021dd3088df6941eb018c997359fb66f2141"];
      ...
      expect(await l1ClaimingService.getMessageProof(TEST_MESSAGE_HASH)).toStrictEqual({
        leafIndex: 0,
        proof: [
          "0x1010101010101010101010101010101010101010101010101010101010101020",
          "0xad3228b676f7d3cd4284a5443f17f1962b36e491b30a40b2405849e597ba5fb5",
          "0xb4c11951957c6f8f642c4af61cd6b24640fec6dc7fc607ee8206a99e92410d30",
          "0x21ddb9a356815c3fac1026b6dec5df3124afbadb485c9ba5a3e3398a04b7ba85",
          "0xe58769b32a1beaf1ea27375a44095a0d1fb664ce2dd358e7fcbfb78c26a19344",
        ],
        root: "0x27ff822675b111601ab06a174f8c021dd3088df6941eb018c997359fb66f2141",
      });
    });
  });
  ...
  describe("estimateClaimMessageGas", () => {
    it("should use the old estimate gas method when proof is not needed", async () => {
      ...
      const estimateClaimGasSpy = jest.spyOn(lineaRollup, "estimateClaimWithoutProofGas").mockResolvedValue(50_000n);
      const estimateClaimWithProofGasSpy = jest.spyOn(lineaRollup, "estimateClaimWithProofGas");
    ...
    it("should use the new estimate gas method when proof is needed", async () => {
      ...
      const estimateClaimWithoutProofGasSpy = jest.spyOn(lineaRollup, "estimateClaimWithoutProofGas");
      const estimateClaimWithProofGasSpy = jest.spyOn(lineaRollup, "estimateClaimGas").mockResolvedValue(50_000n);
  ...
  describe("claimMessage", () => {
    it("should use the old claimMessage method when proof is not needed", async () => {
      ...
      const claimWithoutProofSpy = jest
        .spyOn(lineaRollup, "claimWithoutProof")
        .mockResolvedValue(generateTransactionResponse() as ContractTransactionResponse);
      const claimWithProofSpy = jest.spyOn(lineaRollup, "claim");
    ...
    it("should use the new claimMessageWithProof method when proof is needed", async () => {
      jest.spyOn(l1ClaimingService, "isClaimingNeedingProof").mockResolvedValueOnce(true);
      const claimWithoutProofSpy = jest.spyOn(lineaRollup, "claimWithoutProof");
      const claimWithProofSpy = jest
        .spyOn(lineaRollup, "claim")
        .mockResolvedValue(generateTransactionResponse() as ContractTransactionResponse);
