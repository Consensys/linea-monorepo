...
export class DatabaseCleaningPoller implements IPoller {
  ...
  constructor(
    ...
    this.enabled = config?.enabled ?? DEFAULT_DB_CLEANER_ENABLED;
    this.pollingInterval = config?.cleaningInterval ?? DEFAULT_DB_CLEANING_INTERVAL;
    this.msBeforeNowToDelete =
      (config?.daysBeforeNowToDelete ?? DEFAULT_DB_DAYS_BEFORE_NOW_TO_DELETE) * 24 * 60 * 60 * 1000;
  }
