...
import { testL1NetworkConfig } from "../../utils/testHelpers/constants";
...
describe("EIP1559GasProvider", () => {
  ...
  describe("get1559Fees", () => {
    it("should return fee from cache if currentBlockNumber == cacheIsValidForBlockNumber", async () => {
      ...
      expect(fees).toStrictEqual({
        maxFeePerGas: BigNumber.from(testL1NetworkConfig.claiming.maxFeePerGas),
      });
    });
    ...
    it("should return maxFeePerGas and maxPriorityFeePerGas", async () => {
      ...
      expect(fees).toStrictEqual({
        maxFeePerGas: BigNumber.from(testL1NetworkConfig.claiming.maxFeePerGas),
        maxPriorityFeePerGas: BigNumber.from(31_719_355),
      });
  ...
  describe("get1559FeesOld", () => {
    it("should return fee from cache if currentBlockNumber == cacheIsValidForBlockNumber", async () => {
      jest.spyOn(providerMock, "getBlockNumber").mockResolvedValueOnce(0);
      const fees = await eip1559GasProvider.get1559FeesOld();

      const sendSpy = jest.spyOn(providerMock, "send");
    ...
    it("should throw an error 'FeeEstimationError' if maxPriorityFee is greater than maxFeePerGas", async () => {
      ...
      await expect(eip1559GasProvider.get1559FeesOld()).rejects.toThrow(FeeEstimationError);
      expect(sendSpy).toHaveBeenCalledTimes(1);
    });
    ...
    it("should return maxFeePerGas and maxPriorityFeePerGas", async () => {
      ...
      expect(fees).toStrictEqual({
        maxFeePerGas: BigNumber.from(testL1NetworkConfig.claiming.maxFeePerGas),
        maxPriorityFeePerGas: BigNumber.from(6190953),
      });
