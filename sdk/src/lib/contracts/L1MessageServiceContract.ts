...
import { MESSAGE_CLAIMED_STATUS, MESSAGE_SENT_EVENT_SIGNATURE, MESSAGE_UNKNOWN_STATUS } from "../utils/constants";
import { OnChainMessageStatus } from "../utils/enum";
import { mapMessageSentEventOrLogToMessage } from "../utils/mappers";

export class L1MessageServiceContract extends EIP1559GasProvider {
  public readonly contract: ZkEvmV2;
  ...
  public getContract(contractAddress: string, signer?: Signer): ZkEvmV2 {
    if (this.mode === "read-only") {
      return ZkEvmV2__factory.connect(contractAddress, this.provider);
    }
    ...
    return ZkEvmV2__factory.connect(contractAddress, signer);
  }
  ...
  public async estimateClaimGas(
    ...
    const l1FeeRecipient = feeRecipient || ethers.constants.AddressZero;
    try {
