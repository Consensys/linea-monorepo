...
(definterleaved CN_ABC (CN_A CN_B CN_C))
(definterleaved INDEX_ABC (INDEX_A INDEX_B INDEX_C))
(definterleaved MICRO_STAMP_3 (MICRO_INSTRUCTION_STAMP MICRO_INSTRUCTION_STAMP MICRO_INSTRUCTION_STAMP))
(definterleaved VAL_ABC (VAL_A VAL_B VAL_C))
(definterleaved VAL_ABC_NEW (VAL_A_NEW VAL_B_NEW VAL_C_NEW))
    ...
    (if-not-zero CN_ABC_SORTED
      (if-zero (will-remain-constant! CN_ABC_SORTED)
        (if-zero (will-remain-constant! INDEX_ABC_SORTED)
          (if-not-zero (will-remain-constant! MICRO_STAMP_3_SORTED)
