...
import { serializeTransaction, hexToSignature } from "viem";
...
describe("ViemProvider", () => {
  ...
  describe("getBlock", () => {
    ...
    it("returns null on error", async () => {
      ...
      await expect(provider.getBlock(9999)).resolves.toBeNull();
    });
  });
  ...
  describe("send", () => {
    it("delegates to client.request", async () => {
      publicClient.request.mockResolvedValue("0xresult" as never);
  ...
  describe("broadcastTransaction", () => {
    it("sends raw transaction and returns submission with parsed fields", async () => {
      const fakeSig = hexToSignature(
        "0x" +
      ...
      const signedTx = serializeTransaction(
        { to: TEST_ADDRESS as `0x${string}`, value: 0n, gas: 21000n, nonce: 5, type: "eip1559", chainId: 1 },
        fakeSig,
      );

      publicClient.sendRawTransaction.mockResolvedValue(TEST_TX_HASH as never);
