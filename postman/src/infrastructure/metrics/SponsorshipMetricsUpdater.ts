...
export class SponsorshipMetricsUpdater implements ISponsorshipMetricsUpdater {
  ...
  public async incrementSponsorshipFeePaid(txFee: bigint, direction: Direction): Promise<void> {
    const { wei, gwei } = this.convertTxFeesToWeiAndGwei(txFee);
    this.metricsService.incrementCounter(LineaPostmanMetrics.SponsoredMessagesTotal, { direction }, 1);
    this.metricsService.incrementCounter(LineaPostmanMetrics.SponsorshipFeesWei, { direction }, wei);
    this.metricsService.incrementCounter(LineaPostmanMetrics.SponsorshipFeesGwei, { direction }, gwei);
  }
