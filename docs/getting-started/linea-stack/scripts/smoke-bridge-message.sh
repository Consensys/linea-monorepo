...
#
# Sends one Sepolia L1 `sendMessage` transaction, waits for Postman to claim it
...
[ -n "$L1_TOKEN_BRIDGE" ] && log "L1 TokenBridge: https://sepolia.etherscan.io/address/$L1_TOKEN_BRIDGE"
...
L1_TX_HASH="$(printf '%s\n' "$SEND_RECEIPT" | json_string_field transactionHash)"
echo "$L1_TX_HASH" | grep -qE '^0x[a-fA-F0-9]{64}$' || {
  printf '%s\n' "$SEND_RECEIPT" >&2
  die "cast send did not return a transactionHash"
}
log "l1Tx: $L1_TX_HASH"
log "etherscan: https://sepolia.etherscan.io/tx/$L1_TX_HASH"
...
CLAIM_BLOCK_HEX="$(printf '%s\n' "$CLAIM_RECEIPT" | json_string_field blockNumber)"
...
log "Sepolia L1 tx: https://sepolia.etherscan.io/tx/$L1_TX_HASH"
