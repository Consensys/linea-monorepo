...
#      generated at boot: 2 derived L1 submitter keys (data-submission,
#      finalization) plus static L2 dev keys for message anchoring and
#      liveness. Web3signer mounts this directory at /shared and reads
#      keystores from /shared/web3signer-keys/.
#
...
#
# The user supplies one funded L1 deployer key. account-setup deterministically
# derives separate L1 blob/data-submission and aggregation/finalization keys
# from it, then deploy-contracts funds those derived addresses after all
# contract addresses have been consumed. This keeps first boot simple without
# making coordinator's independent nonce managers share one L1 sender.
#
# DEV ONLY notes:
#   - L2 deployer, message anchoring, and liveness signer keys are pre-baked,
#     public-knowledge dev keys. Same as L2 genesis pre-funded accounts. NEVER
#     reuse on mainnet.
#
set -eu
...
# Coordinator's L2 message-anchoring signer must match the L2 MessageService
# L1_L2_MESSAGE_SETTER_ROLE granted during step 2. This is the pre-funded
# "Message anchorer account" in genesis-besu.json.template.
: "${L2_MESSAGE_ANCHORING_PRIVATE_KEY:=0x4d01ae6487860981699236a58b68f807ee5f17b12df5740b85cf4c4653be0f55}"
...
if [ -f "$OUT_JSON" ]; then
  if ! grep -q '"l1BlobSubmitterPubkey"' "$OUT_JSON" \
    || ! grep -q '"l1FinalizationSubmitterPubkey"' "$OUT_JSON" \
    || ! grep -q '"l2MessageAnchoringPubkey"' "$OUT_JSON"; then
    die "$OUT_JSON was generated by an older quickstart and lacks split signer metadata; run 'docker compose --env-file versions.env --env-file .env --profile stack-partial-prover down -v' before retrying"
  fi
...
# Derive role keys. Users may override these for experiments, but the default
# keeps quickstart to one funded Sepolia key in .env.
if [ -z "${L1_BLOB_SUBMITTER_PRIVATE_KEY:-}" ]; then
  L1_BLOB_SUBMITTER_PRIVATE_KEY="$(cast keccak "linea-stack:l1-blob-submitter:$L1_DEPLOYER_PRIVATE_KEY")"
fi
if [ -z "${L1_FINALIZATION_SUBMITTER_PRIVATE_KEY:-}" ]; then
  L1_FINALIZATION_SUBMITTER_PRIVATE_KEY="$(cast keccak "linea-stack:l1-finalization-submitter:$L1_DEPLOYER_PRIVATE_KEY")"
fi

# Uncompressed secp256k1 public keys (0x + 128 hex). Coordinator config pins
# specific public keys for each web3signer-backed signer slot.
L1_DEPLOYER_PUBKEY="$(cast wallet public-key --private-key "$L1_DEPLOYER_PRIVATE_KEY" | awk '{print $NF}')"
L1_BLOB_SUBMITTER_ADDR="$(cast wallet address --private-key "$L1_BLOB_SUBMITTER_PRIVATE_KEY" | awk '{print $NF}')"
L1_BLOB_SUBMITTER_PUBKEY="$(cast wallet public-key --private-key "$L1_BLOB_SUBMITTER_PRIVATE_KEY" | awk '{print $NF}')"
L1_FINALIZATION_SUBMITTER_ADDR="$(cast wallet address --private-key "$L1_FINALIZATION_SUBMITTER_PRIVATE_KEY" | awk '{print $NF}')"
L1_FINALIZATION_SUBMITTER_PUBKEY="$(cast wallet public-key --private-key "$L1_FINALIZATION_SUBMITTER_PRIVATE_KEY" | awk '{print $NF}')"
L2_MESSAGE_ANCHORING_ADDR="$(cast wallet address --private-key "$L2_MESSAGE_ANCHORING_PRIVATE_KEY" | awk '{print $NF}')"
L2_MESSAGE_ANCHORING_PUBKEY="$(cast wallet public-key --private-key "$L2_MESSAGE_ANCHORING_PRIVATE_KEY" | awk '{print $NF}')"
L2_LIVENESS_SIGNER_ADDR="$(cast wallet address --private-key "$L2_LIVENESS_SIGNER_PRIVATE_KEY" | awk '{print $NF}')"
L2_LIVENESS_SIGNER_PUBKEY="$(cast wallet public-key --private-key "$L2_LIVENESS_SIGNER_PRIVATE_KEY" | awk '{print $NF}')"
...
is_addr "$L1_BLOB_SUBMITTER_ADDR" || die "L1 blob submitter address malformed: $L1_BLOB_SUBMITTER_ADDR"
is_addr "$L1_FINALIZATION_SUBMITTER_ADDR" || die "L1 finalization submitter address malformed: $L1_FINALIZATION_SUBMITTER_ADDR"
is_addr "$L2_MESSAGE_ANCHORING_ADDR" || die "L2 message anchoring address malformed: $L2_MESSAGE_ANCHORING_ADDR"
is_addr "$L2_LIVENESS_SIGNER_ADDR" || die "L2 liveness signer address malformed: $L2_LIVENESS_SIGNER_ADDR"
is_pubkey "$L1_DEPLOYER_PUBKEY" || die "L1 deployer pubkey malformed: $L1_DEPLOYER_PUBKEY"
is_pubkey "$L1_BLOB_SUBMITTER_PUBKEY" || die "L1 blob submitter pubkey malformed: $L1_BLOB_SUBMITTER_PUBKEY"
is_pubkey "$L1_FINALIZATION_SUBMITTER_PUBKEY" || die "L1 finalization submitter pubkey malformed: $L1_FINALIZATION_SUBMITTER_PUBKEY"
is_pubkey "$L2_MESSAGE_ANCHORING_PUBKEY" || die "L2 message anchoring pubkey malformed: $L2_MESSAGE_ANCHORING_PUBKEY"
is_pubkey "$L2_LIVENESS_SIGNER_PUBKEY" || die "L2 liveness signer pubkey malformed: $L2_LIVENESS_SIGNER_PUBKEY"
...
log "L2 liveness signer: $L2_LIVENESS_SIGNER_ADDR"
...
cat > "$OUT_JSON" <<EOF
{
  ...
  "signers": {
    ...
    "l2LivenessSignerAddress": "$L2_LIVENESS_SIGNER_ADDR",
    "l2LivenessSignerPubkey": "$L2_LIVENESS_SIGNER_PUBKEY"
  },
  "l1": {
...
# ─────────────────────────────────────────────────────────────────────────────
#
# Two L1 signers are derived from L1_DEPLOYER_PRIVATE_KEY so blob submission
# and finalization do not share a nonce manager. Message anchoring and liveness
# sign L2 txs and use pre-baked dev keys funded in L2 genesis. Format:
# file-raw YAML — see Web3signer docs for `eth1` signer config.
...
write_keystore "anchoring-signer"        "L2 message-anchoring signer (pre-baked dev key, L2 only)" "$L2_MESSAGE_ANCHORING_PRIVATE_KEY"
write_keystore "data-submission-signer"  "L1 blob/data-submission signer (derived from L1 deployer)" "$L1_BLOB_SUBMITTER_PRIVATE_KEY"
write_keystore "finalization-signer"     "L1 aggregation/finalization signer (derived from L1 deployer)" "$L1_FINALIZATION_SUBMITTER_PRIVATE_KEY"
