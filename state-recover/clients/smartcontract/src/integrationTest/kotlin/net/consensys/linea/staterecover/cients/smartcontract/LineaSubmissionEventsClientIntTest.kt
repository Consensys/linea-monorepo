package net.consensys.linea.staterecover.cients.smartcontract
...
import net.consensys.linea.staterecover.clients.smartcontract.LineaSubmissionEventsClientWeb3jIpml
  ...
  private fun testSubmission(
    ...
    val expectedSubmissionEventsToFind: List<Pair<DataFinalizedV3, List<DataSubmittedV3>>> =
      aggregationsAndBlobs
        .filter { it.aggregation != null }
        .map { (aggregation, aggBlobs) ->
          val expectedDataSubmittedEvents = aggBlobs.chunked(6).map { blobsChunk ->
            DataSubmittedV3(
