...
func buildVanishingFixtureCases() ([]vanishingFixtureCase, []codegen.NamedVanishingSystem, error) {
	...
	add := func(source, name string, sys *wiop.System, honest vanishingAssign, invalid vanishingAssign) error {
		...
		honestProof := buildVanishingProofView(sys, routing, honest)
		var invalidProof *vanishingProofView
		if invalid != nil {
			proof := buildVanishingProofView(sys, routing, invalid)
			invalidProof = &proof
		}
...
func buildVanishingProofView(sys *wiop.System, routing codegen.CoinRouting, assign vanishingAssign) vanishingProofView {
	rt := wiop.NewRuntime(sys)
	assign(&rt)
	...
	return vanishingProofView{
		rounds:         rounds,
		allCoins:       vanishingAllCoins(sys, routing, rt),
...
func vanishingAllCoins(sys *wiop.System, routing codegen.CoinRouting, rt wiop.Runtime) []field.Ext {
	out := make([]field.Ext, routing.TotalRoundCoins)
	for roundIdx, round := range sys.Rounds {
		offset := routing.RoundCoinOffsets[roundIdx]
		for coinIdx, coin := range round.Coins {
			out[offset+coinIdx] = rt.GetCoinValue(coin).AsExt()
		}
	}
	return out
}
...
func writeVanishingBenchProof(out *bytes.Buffer, prefix string, proof vanishingProofView) {
	...
	fmt.Fprintf(out, "var %s_all_coins = [_]ext.Ext{\n", prefix)
	for _, coin := range proof.allCoins {
		fmt.Fprintf(out, "    %s,\n", extValueLiteral(coin))
	}
	fmt.Fprintln(out, "};")
	fmt.Fprintf(out, "const %s_ctx = protocol.Context{ .all_coins = &%s_all_coins, .rounds = &%s_rounds };\n", prefix, prefix, prefix)
	fmt.Fprintln(out)

	fmt.Fprintf(out, "const %s_input = vanishing.CheckInput{\n", prefix)
	fmt.Fprintf(out, "    .ctx = %s_ctx,\n", prefix)
	...
	fmt.Fprintf(out, "fn %s_keepRuntime() void {\n", prefix)
...
func writeVanishingBenchCaseSwitch(out *bytes.Buffer, cases []vanishingFixtureCase) {
	...
	fmt.Fprintln(out, "}")
	fmt.Fprintln(out)
	fmt.Fprintln(out, "pub fn keepRuntime(comptime index: usize, comptime invalid: bool) void {")
	fmt.Fprintln(out, "    if (invalid) {")
	fmt.Fprintln(out, "        return switch (index) {")
	for i, tc := range cases {
		if tc.invalid == nil {
			continue
		}
		fmt.Fprintf(out, "            %d => bench_case_%d_invalid_keepRuntime(),\n", i, i)
	}
	fmt.Fprintln(out, "            else => @compileError(\"vanishing benchmark case does not have an invalid input\"),")
	fmt.Fprintln(out, "        };")
	fmt.Fprintln(out, "    }")
	fmt.Fprintln(out, "    return switch (index) {")
	for i := range cases {
		fmt.Fprintf(out, "        %d => bench_case_%d_keepRuntime(),\n", i, i)
	}
	fmt.Fprintln(out, "        else => @compileError(\"unknown vanishing benchmark case index\"),")
	fmt.Fprintln(out, "    };")
	fmt.Fprintln(out, "}")
