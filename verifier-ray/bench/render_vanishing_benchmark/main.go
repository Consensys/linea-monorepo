...
func main() {
	...
	fmt.Fprintln(&out)
	fmt.Fprintln(&out, "This measures the RISC-V instruction count for the typed vanishing benchmark guest. The proof/system data is embedded as generated typed Zig fixture data, so the reported cycle count is focused on `vanishing.verify` and the minimal guest entry/exit path, not proof byte parsing.")
	fmt.Fprintln(&out)
