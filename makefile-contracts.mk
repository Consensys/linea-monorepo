...
deploy-contracts: LINEA_ROLLUP_TYPE_METHOD:=deploy-linea-rollup-v$(L1_CONTRACT_VERSION)
deploy-contracts:
	cd contracts/; \
	...
	if [ "$(LINEA_PROTOCOL_CONTRACTS_ONLY)" = "false" ]; then \
		$(MAKE) -j7 $(LINEA_ROLLUP_TYPE_METHOD) deploy-token-bridge-l1 deploy-l1-test-erc20 deploy-l2messageservice deploy-token-bridge-l2 deploy-l2-test-erc20 deploy-l2-evm-opcode-tester; \
	else \
		$(MAKE) -j6 $(LINEA_ROLLUP_TYPE_METHOD) deploy-l2messageservice; \
	fi
