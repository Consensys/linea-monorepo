...
riscv-test() {
    ...
    case "$1" in
        clean-all|linker-script|vector-exec|blake-rust-build|blake-rust-json|blake-rust-exec|keccak-rust-build|keccak-rust-json|keccak-rust-exec|build-act4|run-act4|install-zkc|exec-elf|elf-to-json)
            ...
            make -f "$makefile" "$1" "${@:2}"
            ;;
        exec|debug|compile|vector-build|vector-json|zkc-exec|zkc-debug|clean|verify-elf)
...
# Build, convert and execute vector inputs with the generic vector targets
riscv-test vector-build <name>.<ext> VECTOR_ACCEPTS_TO_IN_BYTES=path/to/converter.go
riscv-test vector-json <name>.<ext> VECTOR_ACCEPTS=path/to/vectors.all
...
| `make vector-build TEST=foo.<ext>`                       | Compile the test and build vector helper binaries                                      |
| `make vector-json TEST=foo.<ext> VECTOR_ACCEPTS=foo.all` | Generate vector JSON inputs from a vector file                                         |
...
| `VECTOR_ACCEPTS`             | `""`                                                           | Vector input file consumed by `vector-json`                                                        |
| `VECTOR_N_VECTORS`           | `0`                                                            | Number of vectors selected by `vector-json` (`0` means all when supported by the converter)        |
| `VECTOR_JSON_MODE`           | `dir`                                                          | `dir` for one JSON per vector, `single` for one batched JSON                                       |
| `VECTOR_JSON`                | `$(JSON)`                                                      | Batched JSON path used when `VECTOR_JSON_MODE=single`                                              |
| `VECTOR_JSON_DIR`            | `$(dir $(JSON))vector_json`                                    | JSON directory used when `VECTOR_JSON_MODE=dir`                                                    |
| `VECTOR_ACCEPTS_TO_IN_BYTES` | `""`                                                           | Go converter source built by `vector-build` to transform vector inputs into `IN_BYTES`             |
...
| `BLAKE_ACCEPTS`              | `rust/src/blake/blake10.all`                                   | Blake vector file used by `blake-rust-json`                                                        |
| `BLAKE_N_VECTORS`            | `0`                                                            | Number of Blake vectors to generate (`0` means all)                                                |
...
| `KECCAK_ACCEPTS`             | `rust/src/keccak/keccak.accepts`                               | Keccak vector file used by `keccak-rust-json`                                                      |
| `KECCAK_N_VECTORS`           | `10`                                                           | Number of Keccak vectors compiled into and packed for the Keccak guest                             |
| `KECCAK_JSON`                | `rust/target/riscv64im-unknown-none-elf/release/keccak.json`   | JSON path written by `keccak-rust-json`                                                            |
