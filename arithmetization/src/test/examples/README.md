...
riscv-test vector-json <name>.<ext> VECTOR_FILE=path/to/vectors.all
...
| `make vector-json TEST=foo.<ext> VECTOR_FILE=foo.all`    | Generate vector JSON inputs from a vector file                                         |
...
| `VECTOR_FILE`                | `""`                                                           | Vector input file consumed by `vector-json`; format depends on `VECTOR_FILE_TO_IN_BYTES_GO`        |
...
| `VECTOR_FILE_TO_IN_BYTES_GO` | `""`                                                           | Go converter source built by `vector-build` to transform vector inputs into `IN_BYTES`             |
| `VECTOR_FILE_TO_IN_BYTES_BIN` | `$(BIN)_vector2inbytes`                                       | Compiled vector-to-`IN_BYTES` converter used by `vector-json`                                      |
