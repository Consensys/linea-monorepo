...
Add `export ELF2JSON_WRITE_SECTIONS=true` to `~/.bashrc` or `~/.zshrc` to also write `.sections` files containing the allocated ELF section names copied into each JSON input, one per line.
