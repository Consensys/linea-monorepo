...
(defun (legal-transitions-bit)
  (force-bool (+
                ...
                (* IS_EXTRA         (next (+ IS_EXTRA (is-data)))))))
...
(defun (ripsha-stamp-constancy X)
  (if-not-zero (- RIPSHA_STAMP (+ 1 (prev RIPSHA_STAMP)))
               (remained-constant! X)))
...
(defconstraint index-constancies ()
               (begin
                 (counter-constancy INDEX (phase-sum))
...
(defconstraint smallness-of-last-nBYTES ()
               (if-zero (prev IS_EXTRA)
                        (if-not-zero IS_EXTRA
                                     (begin
                                       (eq! (shift DELTA_BYTE 1) (- (shift nBYTES -3) 1))
                                       (eq! (shift DELTA_BYTE 2) (- (+ 240 (shift nBYTES -3)) 1))))))
...
(defconstraint updating-nBYTES_ACC-and-ensuring-full-limbs ()
               (if-eq (prev (is-data)) 1
                      (if-eq (is-data) 1
                             (begin
                               (eq! nBYTES LLARGE)
                               (eq! nBYTES_ACC (+ (prev nBYTES_ACC) nBYTES))))))

(defconstraint achieving-total-size ()
               (if-eq (prev (is-data)) 1
                      (if-eq (is-result) 1
                             (eq! nBYTES_ACC TOTAL_SIZE))))
