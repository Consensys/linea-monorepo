...
(defconstraint    transient-consistency---FIRST-AGAIN---repeat-transient-row---change-in-address-transient-storage-key-or-transaction
                  ...
                  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                  (begin
                    (if-not-zero (remained-constant   (tcp_full_address) )  (eq!  tcp_FIRST_IN_TXN  1))
                    (if-not-zero (remained-constant   tcp_STORAGE_KEY_HI )  (eq!  tcp_FIRST_IN_TXN  1))
                    (if-not-zero (remained-constant   tcp_STORAGE_KEY_LO )  (eq!  tcp_FIRST_IN_TXN  1))
                    (if-not-zero (remained-constant   tcp_ABS_TX_NUM     )  (eq!  tcp_FIRST_IN_TXN  1))
                    ))
...
(defconstraint    transient-consistency---FIRST-AGAIN---repeat-transient-row---no-change-in-address-transient-storage-key-or-transaction
                  (:guard   (transient-consistency---repeat-transient-row))
                  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                  (if-zero (remained-constant   (tcp_full_address))
                           (if-zero (remained-constant   tcp_STORAGE_KEY_HI)
                                    (if-zero (remained-constant   tcp_STORAGE_KEY_LO)
                                             (if-zero (remained-constant   tcp_ABS_TX_NUM)
