...
start-env: LINEA_ROLLUP_TYPE_METHOD:=deploy-linea-rollup-v$(L1_CONTRACT_VERSION)
start-env:
	...
	if [ "$(SKIP_CONTRACTS_DEPLOYMENT)" = "true" ]; then \
		...
		$(MAKE) $(DEPLOY_CONTRACTS_TARGET) L1_CONTRACT_VERSION=$(L1_CONTRACT_VERSION) LINEA_PROTOCOL_CONTRACTS_ONLY=$(LINEA_PROTOCOL_CONTRACTS_ONLY) LINEA_ROLLUP_TYPE_METHOD=$(LINEA_ROLLUP_TYPE_METHOD); \
	fi

start-env-with-validium:
	$(MAKE) start-env L1_CONTRACT_VERSION=1 LINEA_COORDINATOR_DATA_AVAILABILITY=VALIDIUM LINEA_ROLLUP_TYPE_METHOD=deploy-validium
