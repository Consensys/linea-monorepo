...
describe("Bridge ERC20 Tokens L1 -> L2 and L2 -> L1", () => {
  it.concurrent("Bridge a token from L1 to L2", async () => {
    ...
    logger.info("Token balance of L1 account :", l1TokenBalance.toString());
    ...
    logger.info(`Message anchored : ${JSON.stringify(rollingHashUpdatedEvent)}`);
    ...
    logger.info(`Message claimed on L2 : ${JSON.stringify(claimedEvent)}.`);
    ...
    logger.info("Token balance of L2 account :", l2TokenBalance.toString());
  ...
  it.concurrent("Bridge a token from L2 to L1", async () => {
    ...
    logger.info(`Message claimed on L1 : ${JSON.stringify(claimedEvent)}`);
    ...
    logger.info("Token balance of L1 account :", l1BridgedTokenBalance.toString());
