...
describe("Layer 2 test suite", () => {
  ...
  it.concurrent(
    "Should succeed in conflating multiple blocks and proving on L1",
    async () => {
      const account = await config.getL2AccountManager().generateAccount();
      const dummyContract = config.getL2DummyContract(account);
  ...
  it.concurrent(
    "Should succeed in conflating transactions with large calldata with low gas into multiple L1 blocks",
    async () => {
      const account = await config.getL2AccountManager().generateAccount();
      const dummyContract = config.getL2DummyContract(account);
