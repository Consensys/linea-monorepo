...
const contractMigrationTestSuite = (title: string) => {
  describe(title, () => {
    ...
    it("Check L1 data submission and finalization", async () => {
      ...
      await waitForEvents(lineaRollup, lineaRollup.filters.DataSubmitted(null, currentL2BlockNumber.add(1)), 1_000);
    ...
    it("Check L1 claiming", async () => {
      await waitForEvents(lineaRollup, lineaRollup.filters.L2MerkleRootAdded(), 1_000);
      const { messageHash, messageNumber } = l2Messages[0];
...
export default contractMigrationTestSuite;
