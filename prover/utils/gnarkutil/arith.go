...
func ExpVariableExponent[T zk.Element](api frontend.API, x T, exp T, expNumBits int) T {
	...
	expBits := apiGen.ToBinary(&exp, expNumBits)
	res := zk.ValueOf[T](1)
	...
	for i := len(expBits) - 1; i >= 0; i-- {
		...
		res = apiGen.Select(expBits[i], apiGen.Mul(res, &x), res)
	}
