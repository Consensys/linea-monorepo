...
func ZPrefixProduct(dev *gpu.Device, zVec, ratioVec, tempVec *FrVector) {
	...
	cpHost := make([]uint64, maxChunks*6)
	...
	nc := int(numChunks)
	spHost := make([]uint64, nc*6)
	copy(spHost[:6], cpHost[:6])
	for i := 1; i < nc; i++ {
		prev := *(*fr.Element)(unsafe.Pointer(&spHost[(i-1)*6]))
		cur := *(*fr.Element)(unsafe.Pointer(&cpHost[i*6]))
		var prod fr.Element
		prod.Mul(&prev, &cur)
		*(*fr.Element)(unsafe.Pointer(&spHost[i*6])) = prod
	}
...
func PolyEvalGPU(dev *gpu.Device, v *FrVector, z fr.Element) fr.Element {
	...
	maxChunks := (n + 1023) / 1024
	partialsHost := make([]uint64, maxChunks*6)
	var numChunks C.size_t
...
func combinePolyEvalPartials(partialsHost []uint64, numChunks int, z fr.Element) fr.Element {
	...
	readPartial := func(chunk int) fr.Element {
		var r fr.Element
		for limb := range r {
			r[limb] = partialsHost[chunk*6+limb]
		}
		return r
	}
	if numChunks == 1 {
