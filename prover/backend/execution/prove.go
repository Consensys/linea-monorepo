		...
		func() {
			...
			if cfg.Execution.ProverMode != config.ProverModeProofless {
				...
				proverInps := &zkevm.Witness{
					ExecTraces: req.ConflatedTraceGetter(cfg.Execution.ConflatedTracesDir),
					SMTraces:   smTraces,
				}
...
func mustProveAndPass(
	...
	case config.ProverModeFull:
		...
		return execution.MakeProof(setup, fullZkEvm.WizardIOP, proof, publicInput), setup.VerifiyingKeyDigest()
	default:
