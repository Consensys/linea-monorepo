...
func RunGL(cfg *config.Config, req *GLRequest) error {
	...
	var (
		...
		proofGLFile       = path.Join(cfg.Limitless.SubproofsDir, "GL", string(witnessGL.ModuleName), glProofFileName)
		...
		LPPCommitFile     = path.Join(cfg.Limitless.CommitsDir, string(witnessGL.ModuleName), LPPCommitFileName)
	)
