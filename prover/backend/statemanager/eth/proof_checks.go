...
func checkProofsForAccount(traces []any) error {
	...
	for _, trace := range traces {
		...
		if isWorldStateTrace(trace) {
			...
			if len(curr) > 0 {
				...
				logrus.Infof("checking proof for segment of length %v for account %v", len(curr), accountForWSTrace(trace).Hex())
...
func checkProofsWorldState(traces []any) (oldRootHash, newRootHash Digest, err error) {
...
// and final root hash.
func checkSpliceST(traces []any) (oldRootHash, newRootHash Digest, err error) {
...
func bootstrapVerifierStateFromST(trace any) (vs StorageVerifier) {
...
func bootstrapVerifierStateFromWS(trace any) (vs AccountVerifier) {
