...
func CraftResponse(req *Request) (*Response, error) {
	...
	// newShnarf
	parts := schnarfPartsEIP4844{
		oldSchnarf:       prevShnarf,
		snarkHash:        snarkHash,
		newStateRootHash: newZkRootHash,
		x:                xUnreduced,
		y:                y,
	}
	newShnarf := newSchnarfEIP4844(parts)
...
// schnarfParts wrap the arguments needed to create a new Schnarf by calling
// the NewSchnarf() function.
type schnarfPartsEIP4844 struct {
	oldSchnarf, snarkHash, newStateRootHash, x, y []byte
}

// Returns the new shnarf given, the old shnarf, the snark hash, the new state
// root hash, the x and the y in that order. All over 32 bytes.
func newSchnarfEIP4844(parts schnarfPartsEIP4844) []byte {
	var (
		h              = sha3.NewLegacyKeccak256()
		xBytes, yBytes = parts.x, parts.y
	)
