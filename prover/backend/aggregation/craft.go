...
func collectFields(cfg *config.Config, req *Request) (*CollectedFields, error) {
	...
	for i, decompReqFPath := range req.CompressionProofs {
		...
		// This fields are overwritten after every iteration on purpose. We want
...
func CraftResponse(cfg *config.Config, cf *CollectedFields) (resp *Response, err error) {
	...
	aggregatedProofPublicInput := utils.HexHashHex(
		cf.FinalShnarf,
		cf.ParentStateRootHash,
		utils.FmtIntHex32Bytes(int(cf.ParentAggregationLastBlockTimestamp)),
		utils.FmtIntHex32Bytes(int(cf.FinalTimestamp)),
		utils.FmtIntHex32Bytes(int(cf.LastFinalizedBlockNumber)),
		utils.FmtIntHex32Bytes(int(cf.FinalBlockNumber)),
		cf.LastFinalizedL1RollingHash,
		cf.L1RollingHash,
		utils.FmtIntHex32Bytes(int(cf.LastFinalizedL1RollingHashMessageNumber)),
		utils.FmtIntHex32Bytes(int(resp.L1RollingHashMessageNumber)),
		utils.FmtIntHex32Bytes(l2MsgMerkleTreeDepth),
		utils.HexHashHex(cf.L2MsgRootHashes...),
	)

	resp.AggregatedProofPublicInput = utils.ApplyModulusBn254(aggregatedProofPublicInput)
