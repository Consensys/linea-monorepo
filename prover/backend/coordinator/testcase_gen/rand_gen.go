...
func (g *RandGen) PopulateCoordOutput(p *coordinator.ProverOutput) {
	...
	numBlocks := NumBlock()
	blockInputs := make([]coordinator.BlockData, numBlocks)
...
func (g *RandGen) PopulateBlockData(
	...
	pbi.FromAddresses = coordinator.HexConcat(g.FromAddresses(g.Params.SupTxPerBlock)...)
