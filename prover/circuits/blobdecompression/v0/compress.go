...
func DecompressLZSS(
	...
	blobHeaderBytesLen T,
) (
	payloadPackedBytes []T,
	err error,
) {
	...
	var (
		...
		payloadUnpacked        = make([]T, payloadMaxUnpackedSize)
	)
...
func snarkPackAlign(bs []T) []T {
	var (
		numUnpacked    = len(bs)
		numZeroesToAdd = utils.DivCeil(numUnpacked, 31)
	)

	res := make([]T, 0, numUnpacked+numZeroesToAdd)
	for i := range bs {
		if i%31 == 0 {
			res = append(res, 0)
		}
