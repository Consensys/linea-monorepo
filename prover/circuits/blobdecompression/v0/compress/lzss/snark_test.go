...
func TestNoCompression(t *testing.T) {
	...
	circuit := &DecompressionTestCircuit{
		C:                make([]T, len(c)+inputExtraBytes),
		D:                d,
		Dict:             dict,
...
func TestOutBufTooShort(t *testing.T) {
	...
	circuit := decompressionLengthTestCircuit{
		C: make([]T, len(c)+inputExtraBytes),
		D: make([]T, len(d)-truncationAmount), // not enough room
...
func testCompressionRoundTrip(t *testing.T, d, dict []byte, options ...testCompressionRoundTripOption) {
	...
	circuit := &DecompressionTestCircuit{
		C:                make([]T, len(s.compressed)+s.compressedPaddingLen),
		D:                d,
		Dict:             dict,
		CheckCorrectness: true,
		Level:            s.level,
	...
	if len(circuit.D) > 1024 { // for large circuits, use the test engine
		assert.NoError(t, test.IsSolved(circuit, assignment, ecc.BLS12_377.ScalarField()))
	} else {
		RegisterHints()
