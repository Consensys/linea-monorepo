...
// It returns the length of d as a T; if the decompressed stream doesn't fit in d, dLength will be "-1"
func Decompress(api frontend.API, c []T, cLength T, d, dict []T, level lzss.Level) (dLength T, err error) {
	...
	bytes := make([]T, len(c)-sizeHeader+1)
...
func initAddrTable(api frontend.API, bytes, c []T, wordNbBits int, backrefs []lzss.BackrefType) logderivlookup.Table {
