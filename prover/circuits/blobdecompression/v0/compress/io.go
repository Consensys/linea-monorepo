...
func Pack(api frontend.API, words []zk.WrappedVariable, bitsPerWord int) []zk.WrappedVariable {
...
// it is on the caller to make sure the words are within range
func PackN(api frontend.API, words []zk.WrappedVariable, bitsPerWord, wordsPerElem int) []zk.WrappedVariable {
...
func AssertChecksumEquals(api frontend.API, e []zk.WrappedVariable, checksum zk.WrappedVariable) error {
...
func UnpackIntoBytes(api frontend.API, bytesPerElem int, packed []zk.WrappedVariable) (unpacked []zk.WrappedVariable, nbBytes zk.WrappedVariable, err error) {
...
func ReadNum(api frontend.API, c []zk.WrappedVariable, radix *big.Int) zk.WrappedVariable {
	if len(c) == 0 {
		return 0
	}
...
func ShiftLeft(api frontend.API, slice []zk.WrappedVariable, shiftAmount zk.WrappedVariable) []zk.WrappedVariable {
...
type NumReader struct {
	api         frontend.API
	toRead      []zk.WrappedVariable
	radix       *big.Int
	numBound    *big.Int
	wordsPerNum int
	last        zk.WrappedVariable
}
...
func NewNumReader(api frontend.API, toRead []zk.WrappedVariable, numNbBits, wordNbBits int) *NumReader {
...
func (nr *NumReader) Next() zk.WrappedVariable {
	return nr.next(nil)
}
...
func (nr *NumReader) AssertNextEquals(v zk.WrappedVariable) {
	nr.next(v)
}
...
func (nr *NumReader) next(v zk.WrappedVariable) zk.WrappedVariable {
	if len(nr.toRead) == 0 {
		return 0
	}
