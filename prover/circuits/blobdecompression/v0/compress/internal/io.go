...
func (r *RangeChecker) AssertLessThan(bound uint, c ...zk.WrappedVariable) {

	var check func(zk.WrappedVariable)
	switch bound {
	case 1:
		check = func(v zk.WrappedVariable) { r.api.AssertIsEqual(v, 0) }
	case 2:
...
func (r *RangeChecker) IsLessThan(bound uint, c zk.WrappedVariable) zk.WrappedVariable {
...
func (r *RangeChecker) BreakUpBytesIntoWords(wordNbBits int, bytes ...zk.WrappedVariable) (words, recombined []zk.WrappedVariable) {
	...
	recombined = make([]zk.WrappedVariable, len(words))
	for i := range bytes {
