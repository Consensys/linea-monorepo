...
func packVarBytesBE(api frontend.API, bs []T) T {
	res := T(0)
	for i := range bs {
		if i > 0 {
			res = api.Mul(res, T(1<<8))
		}
...
func packVarByteSliceBE(api frontend.API, bs []T) []T {
	res := make([]T, utils.DivExact(len(bs), wordByteSize))
	for i := range res {
...
// be 64 bytes.
func packVarLimbsBE(api frontend.API, bs []T) T {
	res := T(0)
	base := big.NewInt(1)
	base.Lsh(base, 64)
	for i := range bs {
...
func assignVarByteSlice(data []byte, maxLen int) ([]T, error) {
