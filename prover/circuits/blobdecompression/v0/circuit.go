...
func Allocate(
	...
	return Circuit{
		...
		BlobPackedBytes:          make([]T, blob.MaxUsableBytes),
	}
}
...
func (c *Circuit) Define(api frontend.API) error {
	...
	blobCrumbsPadded381 := make([]T, 4096*crumbsPer381) // public_input.VerifyBlobConsistency expects fr381.Bits - 1 bits per field element, where we have fr377.Bits - 1. Padding needed.
...
func mimcHashAnyGnark(
	...
	for i := range fs {
		...
		case emElement:
			...
			x := packVarLimbsBE(api, []T{f.Limbs[3], f.Limbs[2]})
			h.Write(x)
			x = packVarLimbsBE(api, []T{f.Limbs[1], f.Limbs[0]})
			h.Write(x)
		case T:
			h.Write(f)
		}
