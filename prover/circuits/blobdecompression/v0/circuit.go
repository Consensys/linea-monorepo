...
func (c *Circuit) Define(api frontend.API) error {
	...
	blobBits := internal.PackedBytesToBits(api, c.BlobPackedBytes, fr377.Bits-1)
	// Check the evaluation part of the
	blobBitsPadded381 := make([]frontend.Variable, 4096*(fr381.Bits-1)) // public_input.VerifyBlobConsistency expects fr381.Bits - 1 bits per field element, where we have fr377.Bits - 1. Padding needed.
	const frDelta = fr381.Bits - fr377.Bits
	for i := 0; i < 4096; i++ {
		for j := 0; j < frDelta; j++ {
			blobBitsPadded381[i*(fr381.Bits-1)+j] = 0
		}
		for j := frDelta; j < fr381.Bits-1; j++ {
			blobBitsPadded381[i*(fr381.Bits-1)+j] = blobBits[i*(fr377.Bits-1)+j-frDelta]
		}
	}
	y, err := public_input.VerifyBlobConsistency(api, blobBitsPadded381, c.X, c.Eip4844Enabled)
	if err != nil {
		return fmt.Errorf("main define function : while verifying blob consistency : %w", err)
	}

	cPacked := internal.PackNative(api, blobBits, 1)
	hsh.Reset()
	if err != nil {
		return err
	}
