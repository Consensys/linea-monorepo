...
type PackedBatchesIteration struct {
...
func PackBatches(api frontend.API, nbBatches frontend.Variable, blobPayload, batchLengths []frontend.Variable) (*PackBatchesResult, error) {
	...
	iterations := make([]PackedBatchesIteration, 1+len(blobPayload)/31) // one extra iteration of the main loop to simplify eof handling
	...
	iterations[0] = PackedBatchesIteration{
...
func CheckBatchesSums(api frontend.API, hsh snarkHash.FieldHasher, nbBatches frontend.Variable, blobPayload, batchLengths []frontend.Variable, expectedChecksums []frontend.Variable) error {
	...
	{
		hintIn := make([]frontend.Variable, 1, 1+len(packedBatches.BatchEnds)+len(blobPayload))
		hintIn[0] = nbBatches
		hintIn = append(hintIn, packedBatches.BatchEnds[:]...)
		hintIn = append(hintIn, blobPayload...)
		if partialSums, err = api.Compiler().NewHint(batchesSumChecker.PartialChecksumBatchesPackedHint, len(packedBatches.BatchEnds), hintIn...); err != nil {
			return err
		}
	}
	...
	compressor := hashAsCompressor{hsh}

	iterations := packedBatches.Iterations
	batchSum := iterations[0].Current // normally this should be taken care of by the api.Select(currAlreadyOver,... line. But the very first batch doesn't get this treatment because we know it starts at 0
	for i := 1; i < len(iterations); i++ {
		batchSum = api.Select(
			iterations[i].NoOp,
			batchSum,
			compressor.Compress(batchSum, iterations[i].Current),
		)

		internal.AssertEqualIf(api, iterations[i].NextStarts, batchSum, partialSumsT.Lookup(iterations[i].BatchI)[0]) // if we're done with the current checksum, check that the claimed one from the table is equal to it
		...
		batchSum = api.Select(iterations[i].NextStarts, iterations[i].Next, batchSum) // if the next one starts, the sum is the 31 byte "prefix" of the next batch; assumes any batch is at least 31 bytes long
	}

	// hash along the lengths and compare with expected values
	for i := range batchLengths {
		packedBatches.Range.AssertEqualI(i, expectedChecksums[i], compressor.Compress(batchLengths[i], partialSums[i]))
	}
