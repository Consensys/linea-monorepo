...
func combine(api frontend.API, bytes []T, perNewWord int) []T {
	res := make([]T, len(bytes)/perNewWord)
	for i := range res {
...
func readNum(api frontend.API, bytes []T) T {
...
func parseHeader(api frontend.API, blobBytes []T, blobLen T) (headerLen T, dictHash T, nbBatches T, bytesPerBatch []T, err error) {
	...
	iterateInRange(api, nbBatches, MaxNbBatches, func(i int, inRange T) { // TODO-perf decide whether or not to merge this "loop" with the truncation above. PROBABLY NOT WORTH IT: currently this entire function is not even showing up in the profile graph
...
func CheckBatchesSums(api frontend.API, hasher snarkHash.FieldHasher, nbBatches T, blobPayload []T, batchLengths []T, expectedChecksums []T) error {
	...
	inputExt := make([]T, dummyBatchEnd+31)
	...
	{
		hintIn := make([]T, 1, 1+len(batchEnds)+len(blobPayload))
		hintIn[0] = nbBatches
...
func divBy31(api frontend.API, v []T, bits int) (q, r []T, err error) {
	qNbBits := bits - 4
...
func iterateInRange(api frontend.API, dynamicRange T, staticRange int, f func(i int, inRange T)) {
	inRange := T(1)
...
func crumbStreamToByteStream(api frontend.API, crumbs []T) (bytes []T, nbBytes T) {
...
func ProcessBlob(api frontend.API, hsh snarkHash.FieldHasher, maxUncompressedBlobSize int, blobBytes []T, evaluationChallenge [32]T, eip4844Enabled T, expectedBatchSums internal.VarSlice, dict []T) (blobSum T, evaluation [2]T, err error) {
	...
	payload := make([]T, maxUncompressedBlobSize)
...
func CheckDictChecksum(api frontend.API, checksum T, dict []T) error {
