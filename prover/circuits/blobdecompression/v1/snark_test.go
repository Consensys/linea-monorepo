...
func testChecksumBatches(t *testing.T, blob []byte, batchEndss ...[]int) {
	...
	circuit := testChecksumCircuit{
		Blob: make([]T, len(blob)),
	}
	for _, batchEnds := range batchEndss {
		var sums, lengths [MaxNbBatches]T
		batchStart := 0
		buf := make([]byte, 32)
...
type testParseHeaderCircuit struct {
	Blob                          []T
	BlocksPerBatch                [MaxNbBatches]T
	HeaderLen, NbBatches, BlobLen T
}
...
func TestUnpackCircuit(t *testing.T) {
	...
	runTest := func(b []byte) {
		...
		circuit := unpackCircuit{
			PackedBytes: make([]T, packedBuf.Len()),
			Bytes:       make([]T, len(b)),
		}
