...
func testChecksumSubSlices(t *testing.T, bigSliceLength, lengthsSliceLength int, lengths ...int) {
	...
	endPointsSnark := make([]T, lengthsSliceLength)
	for n := utils.Copy(endPointsSnark, internal.PartialSumsInt(lengths)); n < lengthsSliceLength; n++ {
		endPointsSnark[n] = n * 234
	}

	t.Run(fmt.Sprintf("%d,%d,%v", bigSliceLength, lengthsSliceLength, lengths), snarkTestUtils.SnarkFunctionTest(func(api frontend.API) []T {
...
func TestConcat(t *testing.T) {
	snarkTestUtils.SnarkFunctionTest(func(api frontend.API) []T {
		res := internal.Concat(api, 3, internal.VarSlice{[]T{2}, 1}, internal.VarSlice{[]T{3}, 0})
...
func elementsToEmulated(field *emulated.Field[emulated.BLS12381Fr], s []fr381.Element) []emulated.Element[emulated.BLS12381Fr] {
	return internal.MapSlice(func(element fr381.Element) emulated.Element[emulated.BLS12381Fr] {
		return *field.NewElement(internal.MapSlice(func(x uint64) T { return x }, element[:]...))
	}, s...)
}
...
func TestPartitionSlice(t *testing.T) {
	...
	test := func(slice []T, selectors []int, subsSlack []int) func(*testing.T) {
		assert.Equal(t, len(selectors), len(slice))
		assert.Equal(t, len(subsSlack), nbSubs)

		subs := make([][]T, nbSubs)
		for j := range subs {
			subs[j] = make([]T, 0, sliceLen)
		}
		...
		return snarkTestUtils.SnarkFunctionTest(func(api frontend.API) []T {

			slice := utils.ToVariableSlice(slice)

			subsEncountered := internal.MapSlice(func(s []T) []T { return make([]T, len(s)) }, subs...)
	...
	test([]T{5}, []int{2}, []int{1, 0, 0})(t)
	test([]T{1, 2, 3}, []int{0, 1, 2}, []int{0, 0, 0})
