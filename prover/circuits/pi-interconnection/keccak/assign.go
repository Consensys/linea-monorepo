...
type StrictHasherCircuit struct {
	...
	Ins          [][][2]T // every 32-byte block is prepacked into 2 16-byte blocks
	InLengths    []T      // actual lengths of the inputs
	maxNbKeccakF int
}
...
func allocateIns(lengths []int) [][][2]T {
	ins := make([][][2]T, len(lengths))
	for i := range ins {
		ins[i] = make([][2]T, utils.Abs(lengths[i])/32)
	}
	return ins
}
...
func (h *CompiledStrictHasher) GetCircuit() (c StrictHasherCircuit, err error) {
	...
	c.InLengths = make([]T, len(h.lengths))
...
func (h *StrictHasher) Assign() (c StrictHasherCircuit, err error) {
	...
	c.Ins = make([][][2]T, len(h.ins))
	c.InLengths = make([]T, len(h.ins))
	for i, in := range h.ins {
		c.Ins[i] = make([][2]T, utils.Abs(h.expectedLengths[i])/32) // already checked that the lengths are multiples of 32
		for j := range c.Ins[i] {
			if j < len(in)/32 {
...
func (s *StrictHasherSnark) Sum(nbIn T, bytess ...[32]T) [32]T {
	api := s.h.api
