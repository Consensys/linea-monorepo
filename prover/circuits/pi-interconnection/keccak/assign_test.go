...
func testAssign(t *testing.T, maxSizes []int, actualSizes []int) {
	...
	assignment := testAssignCircuit{
		Ins:   make([][][32]T, len(actualSizes)),
		NbIns: make([]T, len(actualSizes)),
		Outs:  make([][32]T, len(actualSizes)),
	}
	...
	for i := range actualSizes { // for each hash
		...
		assignment.Ins[i] = make([][32]T, maxSize/32)
		circuit.Ins[i] = make([][32]T, len(assignment.Ins[i]))
