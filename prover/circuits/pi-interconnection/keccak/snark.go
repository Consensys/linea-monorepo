...
// bytes are not range-checked. Caller must ensure that bytess[i][j] < 256
// the output are 32 bytes
func (h *Hasher) Sum(nbIn T, bytess ...[32]T) [32]T {

	hintIn := make([]T, 1+32*len(bytess))
	radix := big.NewInt(256)
	unpaddedLanes := make([]T, 4*len(bytess))
	for i := range bytess {
		var currLaneBytes [8]T
		for j := 0; j < 4; j++ {
	...
	h.claimedOuts = append(h.claimedOuts, [2]T{
...
func (h *Hasher) createColumns() (lanes, isLaneActive, isFirstLaneOfNewHash []T) {
	lanes = make([]T, h.nbLanes)
	isLaneActive = make([]T, h.nbLanes)
	isFirstLaneOfNewHash = make([]T, h.nbLanes)
	...
	var (
		currHashI, currHashLaneI                   T = 0, 0
		isCurrLaneActive, isCurrFirstLineOfNewHash T = 1, 1
	)
...
func pad(api frontend.API, inputLanes []T, length T) (lanes []T, nbLanes T) {
	if length == nil {
		nbBlocks := 1 + len(inputLanes)/lanesPerBlock
		lanes = make([]T, nbBlocks*lanesPerBlock) // static length
		nbLanes = len(lanes)
	...
	lanes = make([]T, lanesPerBlock*(1+len(inputLanes)/lanesPerBlock))
...
func divByLanesPerBlock(api frontend.API, x T) (q, r T, err error) {
