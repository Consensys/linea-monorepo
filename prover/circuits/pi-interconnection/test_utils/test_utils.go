...
// AssignSingleBlockBlobNoInvalidity is like AssignSingleBlockBlob but with
// zero invalidity proofs. When used with withSlack(2), the circuit is
// compiled with MaxNbInvalidity > 0, exercising the padding path in
// assignInvalidity.
func AssignSingleBlockBlobNoInvalidity(t require.TestingT) pi_interconnection.Request {
	blob := blobtesting.SingleBlockBlob(t)
	...
	agg := public_input.Aggregation{
		...
		FinalFtxNumber:                          3,
		LastFinalizedFtxRollingHash:             utils.HexEncodeToString(prevFtxRollingHash[:]),
		FinalFtxRollingHash:                     utils.HexEncodeToString(prevFtxRollingHash[:]),
	}
