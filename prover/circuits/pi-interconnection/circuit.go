...
func (c *Circuit) Define(api frontend.API) error {
	...
	nbBatchesSums := rDecompression.PartialSumsF(func(i int) T { return c.DecompressionFPIQ[i].NbBatches })
	...
	batchHashes := make([]T, len(c.ExecutionPublicInput))
	...
	for j := range l2MessagesByByte {
		...
		for k := range l2MessagesByByte[j] {
			l2MessagesByByte[j][k] = internal.VarSlice{Values: make([]T, execMaxNbL2Msg)}
		}
	}
	...
	merkleLeavesConcat := internal.Var32Slice{Values: make([][32]T, c.L2MessageMaxNbMerkle*merkleNbLeaves)}
	for i := 0; i < 32; i++ {
	...
	pi := public_input.AggregationFPISnark{
		...
		L2MsgMerkleTreeRoots: make([][32]T, c.L2MessageMaxNbMerkle), // implicit CHECK_MERKLE_CAP1
		// implicit CHECK_FINAL_NUM
		FinalBlockNumber: rExecution.LastF(func(i int) T { return c.ExecutionFPIQ[i].FinalBlockNumber }),
		// implicit CHECK_FINAL_TIME
		FinalBlockTimestamp:    rExecution.LastF(func(i int) T { return c.ExecutionFPIQ[i].FinalBlockTimestamp }),
...
func MerkleRootSnark(hshK keccak.BlockHasher, leaves [][32]T) [32]T {
...
func allocateCircuit(cfg config.PublicInput) Circuit {
	res := Circuit{
		DecompressionPublicInput: make([]T, cfg.MaxNbDecompression),
		ExecutionPublicInput:     make([]T, cfg.MaxNbExecution),
	...
	for i := range res.ExecutionFPIQ {
		res.ExecutionFPIQ[i].L2MessageHashes.Values = make([][32]T, cfg.ExecutionMaxNbMsg)
	}
