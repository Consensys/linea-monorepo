...
func main() {
	...
	if *flagBound1 != -1 && *flagBound2 != -1 {
		...
		if v > *flagTargetNbConstraints {
			...
			for v < *flagTargetNbConstraints {
				a = b
				b += *flagCrawlStep
				v = nbConstraints(v2.Config{MaxUncompressedNbBytes: b, MaxNbBatches: maxNbBatches})
				*flagCrawlStep *= 2
			}
		}
		if v == *flagTargetNbConstraints {
	...
	for b > a {
		m := (b + a) / 2
		v := nbConstraints(v2.Config{MaxUncompressedNbBytes: m, MaxNbBatches: maxNbBatches})
		if v > *flagTargetNbConstraints {
			b = m
		}
		if v < *flagTargetNbConstraints {
