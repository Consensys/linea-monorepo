...
// TestSingleAggregationPublicInput allows testing a single file with detailed output
func TestSingleAggregationPublicInput(t *testing.T) {
	workspaceRoot := findWorkspaceRoot(t)
	testFile := filepath.Join(workspaceRoot, "contracts/test/hardhat/_testData/compressedDataEip4844/multipleProofs/aggregatedProof-1-81.json")
	...
	t.Log("=== Aggregation Public Input Verification ===")
	t.Log("")
	t.Logf("File: %s", testFile)
	t.Log("")
	t.Log("--- Input Values ---")
	t.Logf("ParentAggregationFinalShnarf:        %s", agg.ParentAggregationFinalShnarf)
	t.Logf("FinalShnarf:                         %s", agg.FinalShnarf)
	t.Logf("ParentAggregationLastBlockTimestamp: %d", agg.ParentAggregationLastBlockTimestamp)
	t.Logf("FinalTimestamp:                      %d", agg.FinalTimestamp)
	t.Logf("LastFinalizedBlockNumber:            %d", agg.LastFinalizedBlockNumber)
	t.Logf("FinalBlockNumber:                    %d", agg.FinalBlockNumber)
	t.Logf("LastFinalizedL1RollingHash:          %s", agg.LastFinalizedL1RollingHash)
	t.Logf("L1RollingHash:                       %s", agg.L1RollingHash)
	t.Logf("LastFinalizedL1RollingHashMsgNum:    %d", agg.LastFinalizedL1RollingHashMessageNumber)
	t.Logf("L1RollingHashMessageNumber:          %d", agg.L1RollingHashMessageNumber)
	t.Log("")
	t.Log("--- FTX Values ---")
	t.Logf("LastFinalizedFtxRollingHash:         %s", agg.LastFinalizedFtxRollingHash)
	t.Logf("FinalFtxRollingHash:                 %s", agg.FinalFtxRollingHash)
	t.Logf("LastFinalizedFtxNumber:              %d", agg.LastFinalizedFtxNumber)
	t.Logf("FinalFtxNumber:                      %d", agg.FinalFtxNumber)
	t.Log("")
	t.Log("--- Other Values ---")
	t.Logf("L2MsgMerkleTreeDepth:                %d", agg.L2MsgMerkleTreeDepth)
	t.Logf("L2MsgRootHashes count:               %d", len(agg.L2MsgRootHashes))
	t.Logf("ChainID:                             %d", agg.ChainID)
	t.Logf("BaseFee:                             %d", agg.BaseFee)
	t.Logf("CoinBase:                            %x", agg.CoinBase)
	t.Logf("L2MessageServiceAddr:                %x", agg.L2MessageServiceAddr)
	t.Log("")
	t.Log("--- Result ---")
	t.Logf("Expected (from JSON):  %s", expectedPI)
	t.Logf("Computed (from code):  %s", computedPI)
	t.Log("")

	if computedPI == expectedPI {
		t.Log("✓ PUBLIC INPUT MATCHES!")
	} else {
		t.Error("✗ PUBLIC INPUT MISMATCH!")
	}
