...
// assignmentBuilder is used to build the assignment of the [Accumulator] module
// and is implemented like a writer.
type assignmentBuilder struct {

	// maxNumProofs stores the maximum number of merkle proof the accumulator
	// module can process in a single execution.
	maxNumProofs int
...
func newAssignmentBuilder(maxNumProofs int) *assignmentBuilder {
	amb := assignmentBuilder{}
	amb.maxNumProofs = maxNumProofs
	amb.leaves = make([]field.Element, 0, maxNumProofs)
	amb.positions = make([]field.Element, 0, maxNumProofs)
	amb.roots = make([]field.Element, 0, maxNumProofs)
	amb.proofs = make([]smt.Proof, 0, maxNumProofs)
	amb.useNextMerkleProof = make([]field.Element, 0, maxNumProofs)
	amb.isActive = make([]field.Element, 0, maxNumProofs)
	amb.accumulatorCounter = make([]field.Element, 0, maxNumProofs)
	amb.isFirst = make([]field.Element, 0, maxNumProofs)
	amb.isInsert = make([]field.Element, 0, maxNumProofs)
	amb.isDelete = make([]field.Element, 0, maxNumProofs)
	amb.isUpdate = make([]field.Element, 0, maxNumProofs)
	amb.isReadZero = make([]field.Element, 0, maxNumProofs)
	amb.isReadNonZero = make([]field.Element, 0, maxNumProofs)
	amb.hKey = make([]field.Element, 0, maxNumProofs)
	amb.hKeyMinus = make([]field.Element, 0, maxNumProofs)
	amb.hKeyPlus = make([]field.Element, 0, maxNumProofs)
	amb.leafMinusIndex = make([]field.Element, 0, maxNumProofs)
	amb.leafMinusNext = make([]field.Element, 0, maxNumProofs)
	amb.leafPlusIndex = make([]field.Element, 0, maxNumProofs)
	amb.leafPlusPrev = make([]field.Element, 0, maxNumProofs)
	amb.leafDeletedIndex = make([]field.Element, 0, maxNumProofs)
	amb.leafDeletedPrev = make([]field.Element, 0, maxNumProofs)
	amb.leafDeletedNext = make([]field.Element, 0, maxNumProofs)
	amb.leafOpening = make([][]field.Element, 4)
	amb.interm = make([][]field.Element, 3)
	for i := 0; i < len(amb.leafOpening); i++ {
		amb.leafOpening[i] = make([]field.Element, 0, maxNumProofs)
	}
	for i := 0; i < len(amb.interm); i++ {
		amb.interm[i] = make([]field.Element, 0, maxNumProofs)
	}
	amb.leafHashes = make([]field.Element, 0, maxNumProofs)
	amb.isEmptyLeaf = make([]field.Element, 0, maxNumProofs)
	amb.nextFreeNode = make([]field.Element, 0, maxNumProofs)
	amb.insertionPath = make([]field.Element, 0, maxNumProofs)
	amb.isInsertRow3 = make([]field.Element, 0, maxNumProofs)
	return &amb
}
...
func (am *Accumulator) Assign(
	...
	builder := newAssignmentBuilder(am.MaxNumProofs)
	paddedSize := am.LeaveSize
	proofPaddedSize := am.ProofSize
...
func (am *Accumulator) assignLeaf(
	run *wizard.ProverRuntime,
	...
	for i := 0; i < len(builder.leafOpening); i++ {
		run.AssignColumn(cols.LeafOpenings[i].GetColID(), smartvectors.RightZeroPadded(builder.leafOpening[i], paddedSize))
	}
...
func (a *assignmentBuilder) computeLeaf(leafOpening accumulator.LeafOpening, isEmptyLeaf bool) {
	if !isEmptyLeaf {
		...
		a.leafOpening[0] = append(a.leafOpening[0], prevFr)
		a.leafOpening[1] = append(a.leafOpening[1], nextFr)
		a.leafOpening[2] = append(a.leafOpening[2], hKeyFr)
		a.leafOpening[3] = append(a.leafOpening[3], hValFr)
		...
		intermTwo := mimc.BlockCompression(intermOne, field.Zero())
		leaf := mimc.BlockCompression(intermTwo, field.Zero())
		a.leafOpening[0] = append(a.leafOpening[0], field.Zero())
		a.leafOpening[1] = append(a.leafOpening[1], field.Zero())
		a.leafOpening[2] = append(a.leafOpening[2], field.Zero())
		a.leafOpening[3] = append(a.leafOpening[3], field.Zero())
		a.interm[0] = append(a.interm[0], intermZero)
		a.interm[1] = append(a.interm[1], intermOne)
		a.interm[2] = append(a.interm[2], intermTwo)
		a.leafHashes = append(a.leafHashes, leaf)
