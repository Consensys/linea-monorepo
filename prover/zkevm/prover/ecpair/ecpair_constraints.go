...
func (ec *ECPair) csInstanceIDChangeWhenNewInstance(comp *wizard.CompiledIOP) {
	...
	comp.InsertGlobal(
		...
		sym.Mul(
			column.Shift(verifiercol.NewConstantCol(field.One(), ec.UnalignedPairingData.IsActive.Size(), true), -1), // this "useless" line helps cancelling the constraint on the first row
			ec.UnalignedPairingData.IsActive,
			ec.UnalignedPairingData.IsFirstLineOfInstance,
			ec.UnalignedPairingData.InstanceID,
			prevEqualCurrID,
