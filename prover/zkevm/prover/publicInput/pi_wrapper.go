...
func NewPublicInput(comp *wizard.CompiledIOP, isInvalidityPI bool, settings *Settings, ss *statesummary.Module, ecdsaModule *ecdsa.EcdsaZkEvm, arith *arithmetization.Arithmetization) *PublicInputs {
	if isInvalidityPI {
		return &PublicInputs{
			ExecutionPI:    nil,
			InvalidityPI:   invalidity.NewInvalidityPI(comp, ecdsaModule, ss, invalidity.GetLogColumns(comp, arith)),
			IsInvalidityPI: true,
		}
	}
	pi := NewPublicInputZkEVM(comp, settings, ss, arith)
	return &PublicInputs{
		ExecutionPI:    &pi,
		InvalidityPI:   nil,
