...
func NewChainIDFetcher(comp *wizard.CompiledIOP, name string, bdc *arith.BlockDataCols) ChainIDFetcher {
	size := bdc.Ct.Size()
	res := ChainIDFetcher{
		ChainID:       util.CreateColBase(name, "CHAIN_ID", size, comp),
...
func DefineChainIDFetcher(comp *wizard.CompiledIOP, fetcher *ChainIDFetcher, name string, bdc *arith.BlockDataCols) {
	...
	comp.InsertLocal(
		...
		sym.Sub(
			...
			// implicitly assume that the ChainID is stored in the last Data column of BlockDataCols
			column.Shift(bdc.Data[len(bdc.Data)-1], ChainIDOffset), // Data at offset -3 (last block's chain ID)
		),
	)
...
func AssignChainIDFetcher(run *wizard.ProverRuntime, fetcher *ChainIDFetcher, bdc *arith.BlockDataCols) {
	...
	fetchedChainID := bdc.Data[len(bdc.Data)-1].GetColAssignmentAt(run, size+ChainIDOffset)
