...
func baseRecomposeBinaryLen(slices []ifaces.Column, base any, lenSlices []ifaces.Column) *symbolic.Expression {
	...
	for k := len(slices) - 1; k >= 0; k-- {
		//currBase := (base * lenSlices[k]+ (1-lenSlices[k]))
...
// It receives a set of numbers and cut/stitch them 8 by 8.
// Example (15,3,6) is organized as (8,7,1,2,6),
// 15 --> 8,7, 3-->1,2 (because stitching 7 and 1 gives 8)
// each number is max 16, thus it would fall max into three parts
func cutAndStitchBy8(nByte []field.Element) (b [maxLanesFromLimb][]field.Element) {
	if maxLanesFromLimb != 3 {
		utils.Panic("the implementation supports only 3 slices")
	}
	missing := uint64(8)
	for i := range nByte {
		var a []field.Element
		...
		// Thus add the zeros at the beginning, if we have less than three slices.
		for len(a) < maxLanesFromLimb {
