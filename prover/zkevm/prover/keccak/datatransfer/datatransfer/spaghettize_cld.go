...
type spaghettizedCLD struct {
	...
	// Note: we need this only due to the limitation that inclusion query does not support expression
	// Once such a support is provided, this matrix can be removed.
	isNewHashMatrix [maxLanesFromLimb]ifaces.Column
...
func (s *spaghettizedCLD) newSpaghetti(
	...
	cldSize := cld.cld[0].Size()
	// Declare the columns
	s.insertCommit(comp, round, cldSize, maxNumRows)
...
func (s *spaghettizedCLD) insertCommit(comp *wizard.CompiledIOP, round, cldSize, maxNumRows int) {
	// declare the columns
	...
	s.isActive = comp.InsertCommit(round, ifaces.ColIDf("IsActive"), maxNumRows)
	...
	for j := range s.isNewHashMatrix {
		s.isNewHashMatrix[j] = comp.InsertCommit(round, ifaces.ColIDf("IsNewHash_Matrix_%v", j), cldSize)
