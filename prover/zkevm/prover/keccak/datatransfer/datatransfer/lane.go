...
type lane struct {
	// All the the lanes of all the blocks
	lane ifaces.Column
	...
	// shifted version of (effective part of) isFirstLaneOfNewHash
...
func (l *lane) newLane(comp *wizard.CompiledIOP,
	...
	l.csCoeff(comp, round, maxRows, sCLD)
	...
	projection.InsertProjection(comp, round, ifaces.QueryIDf("Project_IsFirstLaneOfHash"),
...
func (l *lane) csIsLaneActive(comp *wizard.CompiledIOP, round int, s spaghettizedCLD) {
	...
	comp.InsertLocal(round, ifaces.QueryIDf("Shift_IsLaneComplete_Loc"),
		symbolic.Sub(l.isLaneCompleteShifted, 1))
}
...
func (l *lane) csCoeff(
	...
	//local; coeff[0]=1
	comp.InsertLocal(round, ifaces.QueryIDf("Coeffcient_Loc"), symbolic.Sub(l.coeff, 1))
...
func (l *lane) csRecomposeToLanes(
	...
	projection.InsertProjection(comp, round, ifaces.QueryIDf("ProjectOverLanes"),
		[]ifaces.Column{l.ipTracker},
		[]ifaces.Column{l.lane},
		s.isLaneComplete, l.isLaneActive)
}

// It assign the columns specific to the submodule
func (l *lane) assignLane(
	...
	run.AssignColumn(l.isLaneActive.GetColID(),
		smartvectors.RightZeroPadded(vector.Repeat(field.One(), iPadd.numBlocks*numLanesInBlock), maxNumRowsForLane))

	// assign coeff
	l.assignCoeff(run, sCLD, maxRows)

	// assign the lanes
	l.assignLaneColumn(run, maxNumRowsForLane, permTrace, iPadd, sCLD)
