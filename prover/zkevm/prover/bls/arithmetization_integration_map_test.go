...
func testBlsMapOnTrace(t *testing.T, g Group, path string, limits *Limits) {
	...
	if len(files) == 0 {
		t.Fatal("no lt files found, please extract test files from linea-traces unit tests")
	}
			...
			if cmp == nil || lastMaxLen < maxLen {
				...
				cmp = wizard.Compile(
					func(b *wizard.Builder) {
						...
						blsMap = newMap(b.CompiledIOP, g, limits, newMapDataSource(b.CompiledIOP, g)) // ref zkevm_modules.go
...
func TestBlsG1MapOnTrace(t *testing.T) {
	...
	//testBlsMapOnTrace(t, G1, "testdata/BlsG1MapFpToG1Test_*.lt", limits)
	testBlsMapOnTrace(t, G1, "../testdata_devnet/*/*.lt", limits)
}
...
func TestBlsG2MapOnTrace(t *testing.T) {
	...
	// testBlsMapOnTrace(t, G2, "testdata/BlsG2MapFp2ToG2Test_*.lt", limits)
	testBlsMapOnTrace(t, G2, "../testdata_devnet/*/*.lt", limits)
}
