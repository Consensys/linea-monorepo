...
func nativeScalarMulAndSum(g group, currentAccumulator []field.Element, point []field.Element, scalar []field.Element) (nextAccumulator []field.Element) {
	...
	case G2:
		...
		set(nbL/4, &P.X.A1, point[:nbL/4])
		set(nbL/4, &P.X.A0, point[nbL/4:nbL/2])
		set(nbL/4, &P.Y.A1, point[nbL/2:3*nbL/4])
		set(nbL/4, &P.Y.A0, point[3*nbL/4:])
		set(nbL/4, &C.X.A1, currentAccumulator[:nbL/4])
		set(nbL/4, &C.X.A0, currentAccumulator[nbL/4:nbL/2])
		set(nbL/4, &C.Y.A1, currentAccumulator[nbL/2:3*nbL/4])
		set(nbL/4, &C.Y.A0, currentAccumulator[3*nbL/4:])
		N.ScalarMultiplication(&P, s)
		N.Add(&C, &N)
...
func nativeMillerLoopAndSum(prevAccumulator []field.Element, pointG1 []field.Element, pointG2 []field.Element) (nextAccumulator []field.Element) {
	...
	set(nbG2Limbs/4, &Q.X.A1, pointG2[:nbG2Limbs/4])
	set(nbG2Limbs/4, &Q.X.A0, pointG2[nbG2Limbs/4:nbG2Limbs/2])
	set(nbG2Limbs/4, &Q.Y.A1, pointG2[nbG2Limbs/2:3*nbG2Limbs/4])
	set(nbG2Limbs/4, &Q.Y.A0, pointG2[3*nbG2Limbs/4:])
