...
func genFiles(cmd *cobra.Command, args []string) {
	...
	for i, resp := range blobSubmissionResponses {
		...
		err = json.NewEncoder(f).Encode(blobSubmissionResponses[i])
		if err != nil {
	...
	if aggregationResponses != nil {
		...
		err = json.NewEncoder(f).Encode(aggregationResponses)
		if err != nil {
			printlnAndExit("Unexpected error writing output file: %v", err)
		}
...
func validateParameters(cmd *cobra.Command, args []string) {
