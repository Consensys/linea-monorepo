...
func getParamsFromCLI() (cfg *config.Config, optConfig *mir.OptimisationConfig, traceFPath string, relaxed bool, err error) {
	...
	if len(traceFPathCLI) == 0 {
		return nil, nil, "", false, fmt.Errorf("could not find the trace file path, got %++v", traceFPathCLI)
	}

	if cfg, err = config.NewConfigFromFile(configFPathCLI); err != nil {
		return nil, nil, "", false, fmt.Errorf("could not parse the config: %w", err)
	}
	// Sanity check specified optimisation level makes sense.
	if optimisationLevelCLI >= uint(len(mir.OPTIMISATION_LEVELS)) {
		return nil, nil, "", false, fmt.Errorf("invalid optimisation level: %d", optimisationLevelCLI)
	}
