...
const (
	...
	CodeKilledBySigTERM int = 1137 // When the process is killed by the controller. We purposefully use a non-Unix code.
)
...
func (e *Executor) Run(ctx context.Context, job *Job) (status Status) {
	...
	if status.ExitCode == CodeKilledBySigTERM {
		return status
	}
