...
# Usage:
#   WORKERS=4 ./cmd/controller/controller/sh/run_limitless_opt.sh > ./cmd/controller/controller/logs/meta.log 2>&1
#
# Kill examples:
#   # Bootstrapper / Conglomerator
#   kill -SIGUSR1 $(pgrep -f controller-bootstrapper)
#   kill -SIGTERM $(pgrep -f controller-bootstrapper)
#
#   # Any worker
#   kill -SIGUSR1 $(pgrep -f controller-worker)
#   kill -SIGTERM $(pgrep -f controller-worker)
#
# Debug/Diagnostics:
...
WORKER_CFG="$CONFIG_DIR/config-mainnet-workers.toml" # unified worker config with both gl/lpp enabled
...
shutdown_all() {
  ...
  # Send SIGTERM to tracked controller PIDs
  for pid in "${PIDS[@]:-}"; do
...
for cfg in "$BOOTSTRAP_CFG" "$CONGLO_CFG" "$WORKER_CFG"; do
  if [ ! -f "$cfg" ]; then
...
# Workers (unified: handles both GL and LPP tasks)
for ((i=1; i<=WORKERS; i++)); do
  start_instance "$WORKER_CFG" "worker-$i"
done
