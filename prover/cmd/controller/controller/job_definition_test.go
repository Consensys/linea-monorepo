	...
	testcase := []inpFileNamesCases{
		{
			...
			ExpToLarge:     []string{toLargeM, toLargeWithFailM, toLargeWith2FailsM},
			ExpSuccess:     []string{successM, successWithFailM, successWith2FailsM},
			ExpFailW2:      []string{failM, failWithFailM, failWith2FailsM},
		},
		{
...
func TestCompressionInFileRegexp(t *testing.T) {
	...
	testcase := []inpFileNamesCases{
		...
		{
			Ext: "", Fail: "code", ShouldMatch: false,
			Fnames:    []string{missingCv, missingBCv, missingCCv, etvNoCv, notAPoint, badName},
			Explainer: "M does not pick obviously invalid files",
		},
	}
...
func TestAggregatedInFileRegexp(t *testing.T) {
	...
	var (
		...
		// #nosec G101 -- Not a credential
		respWithFailM = "responses/102-103-getZkAggregatedProof.json"
		// #nosec G101 -- Not a credential
		respWith2FailsM = "responses/102-103-getZkAggregatedProof.json"
	)
	...
	testcase := []inpFileNamesCases{
		...
		{
			Ext: "", Fail: "code", ShouldMatch: false,
			Fnames:    []string{missingContentHash, withEtv, notAPoint, badName},
			Explainer: "M does not pick obviously invalid files",
		},
	}
