...
// If the regexp is provided and non-nil, return the first match and panic if
// there are no matches. If the regexp is not provided, returns the empty
// strings
func stringIfRegexpNotNil(r *regexp2.Regexp, s string) (res string) {
	if r != nil {
		res, err := r.FindStringMatch(s)
		if err != nil || res == nil {
			utils.Panic("could not match `%s` with regexp `%s`. Error? %v", s, r.String(), err)
		}
		return res.String()
	}
	return ""
