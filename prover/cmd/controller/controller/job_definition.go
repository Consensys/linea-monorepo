...
func CompressionDefinition(conf *config.Config) JobDefinition {
	...
	return JobDefinition{
		...
		InputFileRegexp: regexp2.MustCompile(
			fmt.Sprintf(
				`^[0-9]+-[0-9]+-bcv[0-9\.]+-ccv[0-9\.]+-getZkBlobCompressionProof.json(\.failure\.%v_[0-9]+)*$`,
				config.FailSuffix,
			),
			regexp2.None,
		),
...
func AggregatedDefinition(conf *config.Config) JobDefinition {
	...
	return JobDefinition{
		...
		OutputFileTmpl: tmplMustCompile(
			"agreg-output-file",
			"{{.Start}}-{{.End}}-getZkAggregatedProof.json",
		),
		...
		ParamsRegexp: struct {
			...
			ContentHash: regexp2.MustCompile(`[a-fA-F0-9]+(?=-getZk)`, regexp2.None),
		},
