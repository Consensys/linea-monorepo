...
// func IntoGnarkAssignment(msgData []field.Element) []T {
// 	assignedMsg := []T{}
// 	for _, x := range msgData {
// 		assignedMsg = append(assignedMsg, T(x))
// 	}
// 	return assignedMsg
// }
...
// T that can be used to assign a vector of T
// in a circuit or to generate a vector of constant in the circuit definition.
func IntoGnarkAssignment[T zk.Element](msgData []field.Element) []T {
	assignedMsg := make([]T, len(msgData))
	for i := 0; i < len(msgData); i++ {
		assignedMsg[i] = *zk.ValueOf[T](msgData[i])
	}
	return assignedMsg
}
