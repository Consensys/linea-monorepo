...
func TestOpBasicEdgeCases(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			inputs: []SmartVector{
				NewConstant(two, 16),
				LeftPadded(vector.Constant(two, 12), two, 16),
				RightPadded(vector.Constant(two, 12), two, 16),
			},
			expectedRes: NewRegular(vector.Constant(field.NewElement(6), 16)),
			fn:          Add,
		},
		{
			...
			expectedRes: NewRegular(vector.Constant(field.NewElement(8), 16)),
			fn:          Mul,
		},
		{
			...
			inputs: []SmartVector{
				...
				LeftPadded(vector.Constant(two, 12), two, 16),
				RightPadded(vector.Constant(two, 12), two, 16),
				NewRegular(vector.Constant(two, 16)),
			},
			expectedRes: NewRegular(vector.Constant(field.NewElement(8), 16)),
			fn:          Add,
		},
