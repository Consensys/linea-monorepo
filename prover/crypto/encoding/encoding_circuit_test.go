...
func TestEncoding(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		...
		witness.ToEncode1[i] = field.NewFrontendFromKoala(toEncode1[i])
	}
	var toEncode2 [12]field.Element
	for i := 0; i < 12; i++ {
		toEncode2[i].SetRandom()
		witness.ToEncode2[i] = field.NewFrontendFromKoala(toEncode2[i])
	}
...
func TestEncode9WVsToFV(t *testing.T) {
	...
	for i := 0; i < KoalabearChunks; i++ {
		witness.Input[i] = field.NewFrontendFromKoala(decomposed[i])
	}
...
func TestRoundTripBLS9Koalabear(t *testing.T) {
	...
	for i := 0; i < KoalabearChunks; i++ {
		witness.Decomposed[i] = field.NewFrontendFromKoala(decomposed[i])
	}
...
func TestConsistencyNativeVsCircuit(t *testing.T) {
	...
	for iter := 0; iter < 10; iter++ {
		...
		for i := 0; i < 8; i++ {
			witness.KoalaInput[i] = field.NewFrontendFromKoala(input[i])
		}
...
func TestMultipleRoundTrips(t *testing.T) {
	for iter := 0; iter < 5; iter++ {
		...
		witness.Original = original.String()
		for i := 0; i < KoalabearChunks; i++ {
			witness.Decomposed[i] = field.NewFrontendFromKoala(decomposed1[i])
		}
...
func TestEncodingPreservesOrdering(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		witness1.Input[i] = field.NewFrontendFromKoala(input1[i])
	}
	...
	for i := 0; i < 8; i++ {
		witness2.Input[i] = field.NewFrontendFromKoala(input2[i])
	}
...
func BenchmarkEncode8WVsToFVWitness(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		for j := 0; j < 8; j++ {
			witness.Input[j] = field.NewFrontendFromKoala(input[j])
		}
		witness.Output = expected.String()
