...
func init() {
	solver.RegisterHint(FFTInverseBN254)
}
...
// gnarkEvalCanonical evaluate p at z where p represents the polnyomial ∑ᵢp[i]Xⁱ
// (the g stands for gnark)
// /!\ to be used once multi commit is supported /!\
...
func GnarkVerifyCommon(
	...
	for j, selectedColID := range entryList {
		...
		logrus.Infof("checking the opened column #%v", j)
		...
		for i := range selectedColSisDigests {
			...
			if !params.HasNoSisHasher() {
				selectedColSisDigests[i][j] = params.Key.GnarkHash(api, selectedSubCol)
			} else {
				hasher, _ := params.NoSisHasher(api)
				hasher.Reset()
				hasher.Write(selectedSubCol...)
				digest := hasher.Sum()
				selectedColSisDigests[i][j] = []frontend.Variable{digest}
			}
...
func GnarkVerify(
	...
	// Consistency computed hashes <-> commitment

	buf := make([]frontend.Variable, hashSize)
	for i, com := range commitments {
		...
		for j, entry := range entryList {
			...
			// select com[entry*hashSize:(entry+1)*hashSize]
			unfoldEntry := api.Mul(entry, hashSize)
			for k := 0; k < hashSize; k++ {
				buf[k] = tbl.Lookup(unfoldEntry)[0]
				unfoldEntry = api.Add(unfoldEntry, 1)
				api.AssertIsEqual(buf[k], selectedColsHashes[i][j][k])
			}
		}
	}
