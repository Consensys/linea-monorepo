...
func mimcHintfunc(f *big.Int, inputs []*big.Int, outputs []*big.Int) error {
	...
	outF := poseidon2.BlockCompression(inpF[0], inpF[1])
...
func (f *HasherFactory) finalize(_ frontend.API) error {
	...
	if len(f.blocks) == 1 {
		new := poseidon2.GnarkBlockCompression(f.api, f.initStates[0], f.blocks[0])
...
func (f *HasherFactory) padToPow2() int {
	...
	zero := field.Element{}
	hashOfZero := poseidon2.BlockCompression(zero, zero)
