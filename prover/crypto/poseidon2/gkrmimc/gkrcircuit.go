...
var (
	...
	numGates     int = prefetchSize + len(poseidon2.Constants)
	gateNames    []string
)
...
func registerGates() {
	...
	for i := 4; i < numGates-1; i++ {
		...
		var (
			//name  = gateNames[i]
			gateG = NewRoundGateGnark(poseidon2.Constants[i-prefetchSize])
	...
	var (
		...
		gateG = NewFinalRoundGateGnark(poseidon2.Constants[len(poseidon2.Constants)-1])
