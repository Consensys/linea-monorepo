...
type Tree struct {
	...
	EmptyNodes []bn254fr.Element
}

// EmptyLeaf returns an empty leaf (e.g. the zero value).
func EmptyLeaf() bn254fr.Element {
...
func hashLR(nodeL, nodeR bn254fr.Element) bn254fr.Element {
	var d bn254fr.Element
	hasher := poseidon2_bn254.NewMDHasher()
...
func NewEmptyTree(depth int) *Tree {
	...
	emptyNodes := make([]bn254fr.Element, depth-1)
	prevNode := EmptyLeaf()
	...
	return &Tree{
		Depth:          depth,
		Root:           root,
		OccupiedLeaves: make([]bn254fr.Element, 0),
		OccupiedNodes:  make([][]bn254fr.Element, depth-1),
		EmptyNodes:     emptyNodes,
	}
}

func (t *Tree) GetRoot() bn254fr.Element {
	return t.Root
}

// GetLeaf returns a leaf by position or an error if the leaf is out of bounds.
func (t *Tree) GetLeaf(pos int) (bn254fr.Element, error) {
	// Check that the accessed node is within the bounds of the SMT
	maxPos := 1 << t.Depth
	if pos >= maxPos {
		return bn254fr.Element{}, fmt.Errorf("out of bound: %v", pos)
	}

	if pos < 0 {
		return bn254fr.Element{}, fmt.Errorf("negative position: %v", pos)
	}
...
func (t *Tree) MustGetLeaf(pos int) bn254fr.Element {
	l, err := t.GetLeaf(pos)
	if err != nil {
...
//
// (for config.Depth == 40)
func (t *Tree) getNode(level, posInLevel int) bn254fr.Element {
	switch {
	case level == t.Depth:
...
//
// (for config.Depth == 40)
func (t *Tree) updateNode(level, posInLevel int, newVal bn254fr.Element) {
	switch {
	case level == t.Depth:
		// The only logical posInLevels value is zero in this case
		if posInLevel > 0 {
...
func (t *Tree) reserveLevel(level, newSize int) {
	...
	if level == 0 {
		...
		padding := make([]bn254fr.Element, newSize-len(t.OccupiedLeaves))
		for i := range padding {
			padding[i] = EmptyLeaf()
		}
	...
	*/
	...
	padding := make([]bn254fr.Element, newSize-len(t.OccupiedNodes[level-1]))
	for i := range padding {
...
func BuildComplete(leaves []bn254fr.Element) *Tree {
	...
	for i := 0; i < depth-1; i++ {
		nextLevel := make([]bn254fr.Element, len(currLevels)/2)
