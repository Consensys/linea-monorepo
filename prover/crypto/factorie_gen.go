...
import (
	...
	"github.com/consensys/linea-monorepo/prover/crypto/gnark_hash"
...
type ExternalHasherFactoryGen struct {
	Api frontend.API
}

// ExternalHasher is an implementation of the [ghash.StateStorer] interface
// that tags the variables happening in a MiMC hasher claim.
type ExternalHasherGen[T zk.Element] struct {
	api   zk.APIGen[T]
	data  []T
	state T
}
...
// interface.
type ExternalHasherBuilderGen[T zk.Element] struct {
	storeCommitBuilderGen[T]
...
type externalHashBuilderIFace[T zk.Element] interface {
	CheckHashExternally(oldState, block, newState T)
}
...
type storeCommitBuilderGen[T zk.Element] interface {
	frontend.Builder[constraint.U32]
	frontend.Committer
	SetKeyValue(key, value any)
	GetKeyValue(key any) (value any)
	GetWiresConstraintExact(wires []T, addMissing bool) ([][2]int, error)
}
