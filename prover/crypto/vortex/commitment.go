...
func (p *Params) CommitMerkle(ps []smartvectors.SmartVector) (encodedMatrix EncodedMatrix, tree *smt.Tree, colHashes []field.Element) {
	...
	codewords := make([]field.Element, numRows*sizeCodeWord)
	encodedMatrix = make([]smartvectors.SmartVector, numRows)

	parallel.Execute(numRows, func(start, end int) {
		for i := start; i < end; i++ {
			params.EncodeReedSolomon(input[i], codewords[i*sizeCodeWord:(i+1)*sizeCodeWord])
			encodedMatrix[i] = smartvectors.NewRegular(codewords[i*sizeCodeWord : (i+1)*sizeCodeWord])
		}
	})
