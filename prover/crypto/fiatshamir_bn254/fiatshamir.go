...
type FS struct {
	...
	h        *poseidon2_bn254.MDHasher
}

func NewFS(verboseMode ...bool) *FS {
	return &FS{h: poseidon2_bn254.NewMDHasher(verboseMode...)}
}

// --------------------------------------------------
// List of methods to updae the FS state with frElmts
func (fs *FS) UpdateFrElmt(vec ...bn254fr.Element) {
	fs.flushKoala()
	fs.h.WriteElements(vec...)
}

func (fs *FS) UpdateVecFrElmt(vecs ...[]bn254fr.Element) {
	for i := range vecs {
		fs.UpdateFrElmt(vecs[i]...)
	}
}

func (fs *FS) RandomFieldFrElmt() bn254fr.Element {
	fs.flushKoala()
...
func (fs *FS) RandomField() field.Octuplet {
	...
	return encoding.EncodeBN254FrElementToOctuplet(r)
}
...
func (fs *FS) SetState(s field.Octuplet) {
	...
	state := encoding.EncodeKoalabearOctupletToBN254FrElement(s)
	fs.h.SetStateFrElement(state)
}

func (fs *FS) State() field.Octuplet {
	state := fs.h.State()
	return encoding.EncodeBN254FrElementToOctuplet(state)
}

func (fs *FS) safeguardUpdate() {
	fs.UpdateFrElmt(bn254fr.Element{})
}
