...
// VerifyBatchedWith is like [VerifyBatched] but uses a pre-supplied lambda
// (the transcript already absorbed it) rather than deriving it from claims.
func VerifyBatchedWith(claims []Claim, proof BatchedProof, lambda fext.Element, t Transcript) (point []fext.Element, polyEvals []fext.Element, err error) {
	N := len(claims)
	if N == 0 {
		return nil, nil, fmt.Errorf("sumcheck.VerifyBatchedWith: no claims")
	}
	n := len(claims[0].Point)
	if n == 0 {
		return nil, nil, fmt.Errorf("sumcheck.VerifyBatchedWith: 0-variable claims are not supported")
	}
	for i := range claims {
		if len(claims[i].Point) != n {
			return nil, nil, fmt.Errorf("sumcheck.VerifyBatchedWith: claim %d has %d vars, expected %d",
				i, len(claims[i].Point), n)
		}
	}
	if len(proof.RoundPolys) != n {
		return nil, nil, fmt.Errorf("sumcheck.VerifyBatchedWith: proof has %d round polys, expected %d",
			len(proof.RoundPolys), n)
	}
	if len(proof.FinalEvals) != N {
		return nil, nil, fmt.Errorf("sumcheck.VerifyBatchedWith: proof has %d final evals, expected %d",
			len(proof.FinalEvals), N)
	}
	...
	for k := 0; k < n; k++ {
		...
		sum01.Add(&g0, &g1)
		if !sum01.Equal(&target) {
			return nil, nil, fmt.Errorf("sumcheck.VerifyBatchedWith: round %d consistency check failed", k)
		}
	...
	if !expected.Equal(&target) {
		return nil, nil, fmt.Errorf("sumcheck.VerifyBatchedWith: final consistency check failed")
	}
