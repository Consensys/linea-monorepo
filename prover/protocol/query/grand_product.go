...
func (g GrandProduct) Compute(run ifaces.Runtime) fext.GenericFieldElem {
	...
	for size := range g.Inputs {
		...
		// numertaors are processed, now process denominators
...
func (g GrandProduct) Check(run ifaces.Runtime) error {
	...
	if params.IsBase {
		...
		actualProdElem, _ := actualProd.GetBase()
		if actualProdElem != params.BaseY {
			return fmt.Errorf("the grand product query %v is not satisfied, actualProd = %v, param.Y = %v", g.ID, actualProd.String(), params.BaseY.String())
		}
	} else {
		for size := range g.Inputs {
			input := g.Inputs[size]
