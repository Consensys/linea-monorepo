...
func (p Poseidon2Gen[T]) Name() ifaces.QueryID {
	return p.ID
}
...
*/
func NewPoseidon2Gen[T zk.Element](id ifaces.QueryID, block, oldState, newState [8]ifaces.ColumnGen[T], selector ifaces.ColumnGen[T]) Poseidon2Gen[T] {
...
*/
func (p Poseidon2Gen[T]) Check(run ifaces.Runtime) error {
...
func (p Poseidon2Gen[T]) CheckGnark(api frontend.API, run ifaces.GnarkRuntimeGen[T]) {
	...
	for i := 0; i < len(newStates); i++ {
		...
		recomputed := poseidon2.GnarkBlockCompressionMekleGen[T](api, oldState, block)
		api.AssertIsEqual(newState, recomputed)
	}
}

func (p Poseidon2Gen[T]) UUID() uuid.UUID {
	return p.uuid
}
