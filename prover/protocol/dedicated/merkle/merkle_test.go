...
type merkleTestBuilder struct {
	proofs             []smt.Proof
	pos                []field.Element
	roots              [][8]field.Element
	leaves             [][8]field.Element
...
func newMerkleTestBuilder(numProofs int) *merkleTestBuilder {
	return &merkleTestBuilder{
		...
		roots:              make([][8]field.Element, 0, numProofs),
		leaves:             make([][8]field.Element, 0, numProofs),
...
func TestMerklePow2(t *testing.T) {
	...
	define := func(b *wizard.Builder) {
		...
		merkle.MerkleProofCheck(b.CompiledIOP, "TEST", depth, numProofs, rootscol, leavescol, proofcol, poscol)
	}

	prove := func(run *wizard.ProverRuntime) {
		fmt.Printf("builder.roots=%v\n", len(builder.roots))
...
func transpose(v [][8]field.Element) [8][]field.Element {
	var transposed [8][]field.Element

	for i := 0; i < 8; i++ {
		transposed[i] = make([]field.Element, len(v))
	}
