...
// interpolationProverAction is the action to assign the interpolation columns.
...
type interpolationProverAction struct {
	...
	p            ifaces.Column
	i            ifaces.Column
	localOpenEnd query.LocalOpening
}

// Run executes the interpolationProverAction over a [ProverRuntime]
func (a *interpolationProverAction) Run(assi *wizard.ProverRuntime) {
	...
	assi.AssignLocalPoint(ifaces.QueryIDf("%v_%v", a.name, INTERPOLATION_OPEN_END), witi[n-1])
}
