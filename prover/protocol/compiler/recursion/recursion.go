...
// efficiency.
type Recursion[T zk.Element] struct {
...
// before the opening.
func (r *Recursion[T]) GetStoppingRound() int {
...
func DefineRecursionOf[T zk.Element](comp, inputComp *wizard.CompiledIOP[T], params Parameters) *Recursion[T] {
	...
	var (
		plonkCircuit = AllocRecursionCircuit[T](inputComp, params.WithoutGkr, params.WithExternalHasherOpts)
	...
	comp.RegisterVerifierAction(0, &ConsistencyCheck[T]{Ctx: rec, PIs: plonkCtx.Columns.PI})
	comp.RegisterProverAction(1, AssignVortexUAlpha[T]{Ctxs: rec})
	comp.RegisterProverAction(2, AssignVortexOpenedCols[T]{Ctxs: rec})
...
func (r *Recursion[T]) Assign(run *wizard.ProverRuntime[T], _wit []Witness, _filling *Witness) {
	...
	for i := range wit {
		...
		var (
			...
			assign = AssignRecursionCircuit[T](r.InputCompiledIOP, wit[i].Proof, wit[i].Pub, wit[i].FinalFS)
		)
		...
		for j := range assign.Commitments {
			...
			// x := assign.Commitments[j].(field.Element) // TODO @thomas fixme (casting T -> real type)
			// run.AssignColumn(colName, smartvectors.NewConstant(x, 1)) // TODO @thomas fixme (casting T -> real type)
		}
...
func (rec *Recursion[T]) GetPublicInputOfInstance(run wizard.Runtime, name string, inst int) field.Element {
...
func (rec *Recursion[T]) GetPublicInputOfInstanceGnark(api frontend.API, run wizard.GnarkRuntime, name string, inst int) T {
	name = addPrefixToID(rec.Name+"-"+strconv.Itoa(inst), name)
