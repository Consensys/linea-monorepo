...
func TestPocNewGlobalTest(t *testing.T) {
	...
	definer := func(build *wizard.Builder[zk.NativeElement]) {
		...
		expr := symbolic.Sub[zk.NativeElement](P, symbolic.Add[zk.NativeElement](column.Shift(P, -1), column.Shift(P, -2)))
...
// 	constant := accessors.NewConstant[zk.NativeElement](field.NewElement(2))
