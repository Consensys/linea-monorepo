...
func (pa EvaluationProver) Run(run *wizard.ProverRuntime) {
	...
	*/
	...
	var (
		...
		mulGenInv         = fft.GetDomainFromCache(uint64(maxRatio * pa.DomainSize)).FrMultiplicativeGenInv
...
func (ctx EvaluationVerifier) recombineQuotientSharesEvaluation(run wizard.Runtime, r fext.Element) ([]fext.Element, error) {
	...
	var (
		...
		mulGenInv = fft.GetDomainFromCache(uint64(maxRatio * ctx.DomainSize)).FrMultiplicativeGenInv
