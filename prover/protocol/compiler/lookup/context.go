...
type singleTableCtx struct {
	...
	// SBoard is as S but stores the corresponding [symbolic.ExpressionBoard].
	// The [symbolic.ExpressionBoard] are only used during the prover runtime
	// but we save computation by storing them
	SBoard []symbolic.ExpressionBoard
	...
	// TBoard is as T but stores the [symbolic.ExpressionBoard]
	TBoard []symbolic.ExpressionBoard

	// SigmaS stores the columns computing incrementally the sum of the inverses
	// if S + Gamma as specified in the log-derivative lookups paper (for each
	// S separately).
	SigmaS []ifaces.Column

	// SigmaT stores the column computing incrementally the sum of the inverses
	// of T + Gamma as specified in the log-derivative lookups paper.
	SigmaT []ifaces.Column

	// SigmaSOpenings are [query.LocalOpening] queries to the end of each SigmaS
	// for the verifier to check the consistency between the log-derivative sums
	// of the Ss and T.
	SigmaSOpenings []query.LocalOpening

	// SigmaSOpening is as SigmaSOpening but for the SigmaT.
