...
type proverTaskAtRound struct {
	...
	// SigmaAssignmentTasks lists all the tasks consisting of assigning the
	// columns SigmaS and SigmaT for the given round.
	SigmaAssignmentTasks []sigmaAssignmentTask
}
...
func (p proverTaskAtRound) Run(run *wizard.ProverRuntime) {
	...
	for i := range p.SigmaAssignmentTasks {
		// the passing of the index `i` is there to ensure that the go-routine
		// is running over a local copy of `i` which is not incremented every
		// time the loop goes to the next iteration.
		go func(i int) {
			...
			p.SigmaAssignmentTasks[i].run(run)
		}(i)
	}
...
// pushSigmaAssignment appends an [sigmaAssignmentTask] to the list of tasks
func (p *proverTaskAtRound) pushSigmaAssignment(s sigmaAssignmentTask) {
	p.SigmaAssignmentTasks = append(p.SigmaAssignmentTasks, s)
}

// numTasks returns the total number of tasks that are scheduled in the
// [proverTaskAtRound].
func (p *proverTaskAtRound) numTasks() int {
	return len(p.MAssignmentTasks) + len(p.SigmaAssignmentTasks)
}
...
// sigmaAssignmentTask represents a prover task of assignming the columns
// SigmaS and SigmaT for a specific lookup table.
type sigmaAssignmentTask singleTableCtx

func (s sigmaAssignmentTask) run(run *wizard.ProverRuntime) {

	var (
		// gamma stores the assignement to the coin gamma as it was sampled
		// during the protocol execution.
		gamma = run.GetRandomCoinField(s.Gamma.Name)

		// m stores the pre-assigned value of m as it was obtained during the
		// previous version of the protocol. It is crucial that this values
		// stays unmuted during the execution of this function as this may have
		// side-effect on the assignment (retro-modification of the witness)
		m = make([][]field.Element, len(s.T))

		// t stores the assignment to the (possibly collapsed with alpha) column
		// T. It is crucial that this values stays unmuted during this function
		// because this may have side-effect (retro-modification of the witness)
		t = make([][]field.Element, len(s.T))
	)

	for frag := range s.T {

		m[frag] = s.M[frag].GetColAssignment(run).IntoRegVecSaveAlloc()
		t[frag] = wizardutils.EvalExprColumn(run, s.TBoard[frag]).IntoRegVecSaveAlloc()
		sigmaT := make([]field.Element, len(t[frag]))

		for k := range sigmaT {
			sigmaT[k].Add(&gamma, &t[frag][k])
		}

		sigmaT = field.BatchInvert(sigmaT)

		for k := range sigmaT {
			sigmaT[k].Mul(&sigmaT[k], &m[frag][k])
			if k > 0 {
				sigmaT[k].Add(&sigmaT[k], &sigmaT[k-1])
			}
		}

		run.AssignColumn(s.SigmaT[frag].GetColID(), sv.NewRegular(sigmaT))
		run.AssignLocalPoint(s.SigmaTOpening[frag].ID, sigmaT[len(sigmaT)-1])
	}

	for i := range s.SigmaS {

		var (
			// sCol stores the (possibly collapsed by alpha) assignement to S[i]. It
			// is important that this slice is not mutated during the evaluation
			// of sigmaS as this may have unintended side-effects on the other
			// columns.
			sCol = wizardutils.EvalExprColumn(run, s.SBoard[i]).IntoRegVecSaveAlloc()

			// sigmaS stores the constructed witness as it is computed
			sigmaS = make([]field.Element, s.SigmaS[i].Size())

			// sFilter stores the assignment of SFilter[i] if a non-nil value
			// is provided or is nil.
			sFilter []field.Element

			// hasFilter indicates whether the filter was provided.
			hasFilter = s.SFilters[i] != nil
		)

		if hasFilter {
			sFilter = s.SFilters[i].GetColAssignment(run).IntoRegVecSaveAlloc()
		}

		for k := range sigmaS {
			sigmaS[k].Add(&gamma, &sCol[k])
		}

		sigmaS = field.BatchInvert(sigmaS)

		if hasFilter {
			for k := range sigmaS {
				sigmaS[k].Mul(&sigmaS[k], &sFilter[k])
			}
		}

		for k := range sigmaS {
			if k > 0 {
				sigmaS[k].Add(&sigmaS[k], &sigmaS[k-1])
			}
		}

		run.AssignColumn(s.SigmaS[i].GetColID(), sv.NewRegular(sigmaS))
		run.AssignLocalPoint(s.SigmaSOpenings[i].ID, sigmaS[len(sigmaS)-1])
	}
