...
func TestVortexSingleRoundMerkle(t *testing.T) {
	...
	compiled := wizard.Compile(define, vortex.Compile(4, vortex.MerkleMode))
...
func TestVortexOneFullOneDryMerkle(t *testing.T) {
	...
	compiled := wizard.Compile(
		...
		vortex.Compile(
			4,
			vortex.MerkleMode,
			vortex.WithSISParams(&ringsis.Params{LogTwoBound_: 8, LogTwoDegree: 6}),
		),
	)
	proof := wizard.Prove(compiled, prove)
	valid := wizard.Verify(compiled, proof)
...
func TestVortexSingleRoundMerkleNoSis(t *testing.T) {
	...
	compiled := wizard.Compile(define, vortex.Compile(4, vortex.MerkleMode, vortex.ReplaceSisByMimc()))
	proof := wizard.Prove(compiled, prove)
	valid := wizard.Verify(compiled, proof)
