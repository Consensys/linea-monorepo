...
func (ctx *SelfRecursionCtx) registersAh() {
	...
	for i, comsInRoundsI := range ctx.VortexCtx.CommitmentsByRounds.Inner() {
		...
		if (len(comsInRoundsI) == 0) != (ctx.Columns.Dh[i] == nil) {
	...
	// the end
	for len(ctx.Columns.Ah) < len(ctx.Columns.Dh) {
		ctx.Columns.Ah = append(ctx.Columns.Ah, nil)
	}

	for len(ctx.Columns.Dh) < len(ctx.Columns.Ah) {
		ctx.Columns.Dh = append(ctx.Columns.Dh, nil)
	}
