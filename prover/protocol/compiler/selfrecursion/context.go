...
type Columns struct {
	...
	ConcatenatedDhQ [blockSize]ifaces.Column // no need to extend to blockSize here
...
// Poseidon2hMetaData is a structure that holds the parameters
...
type Poseidon2MetaData struct {
	...
	// hashPreimages[i] stores the concatenated preimages of all Poseidon2 hashes
	// of the selected columns for the ith non SIS round
	ConcatenatedHashPreimages [][blockSize]ifaces.Column // no need to extend to blockSize here
}
...
type SelfRecursionCtx struct {
	...
	Columns Columns
	// Poseidon2MetaData contains the metadata for the Poseidon2 hash check
	Poseidon2MetaData Poseidon2MetaData
}
