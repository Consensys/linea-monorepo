...
type Columns struct {
	...
	// Roots, Leaves and Proofs are generated by poseidon2 hash, we store them in block of 8
	...
	// (Proof, already computed)
	//
	// WholePreimagesNonSis[round][j] is lane j of the 8-lane layout for the
	// round-th non-SIS round. round=0 is the precomputed non-SIS round (when
	// present), then committed non-SIS rounds in ascending order.
	// lane[j][k*colChunksPad+c] = poly_{c*8+j}(Q_k) for k<NbColsToOpen, c*8+j<colSize, else 0.
	WholePreimagesNonSis [][blockSize]ifaces.Column
	...
	// - ConcatenatedSisHashQ, concatenated as input of the linear combination verification
	...
	ConcatenatedSisHashQ ifaces.Column
	// Represents the input of the poseidon2 hash
	SisHashToHash [blockSize]ifaces.Column
	...
	// Roots, Leaves and Proofs are generated by poseidon2 hash, we store them in block of 8
	SisRoundLeaves [blockSize][]ifaces.Column
	...
	// (Auto-computed)
	//
	// The folding of ConcatenatedSisHashQ by rCollapse
	CollapsedSisHashQ ifaces.Column
...
// NonSisMetaData is a structure that holds the parameters
// for the linear hash check for the non SIS rounds
type NonSisMetaData struct {
	// Contains the number of polynomials for each non SIS round.
	// The size of polynomials can be different for each round
	ToHashSizes []int

	// It provides the "colChunks" for the CheckLinearHash()
	// for each non SIS round
	// as poseidon2 hash takes 8 elements as input, we need to
	// split each column of polynomials into chunks of size 8.
	ColChunks []int
	// NonSisLeaves[i] is the concatenated leaves, generated by
	// the Poseidon2 hashing all the selected columns for the ith non SIS round
	NonSisLeaves [blockSize][]ifaces.Column
}
...
type SelfRecursionCtx struct {
	...
	Columns Columns
	// NonSisMetaData contains the metadata for the Poseidon2 hash check
	NonSisMetaData NonSisMetaData
}
...
func NewRecursionCtx(comp *wizard.CompiledIOP, vortexCtx *vortex.Ctx, prefix string) SelfRecursionCtx {
	...
	// WholePreimagesNonSis is [round][query], iterate both dimensions.
	for _, roundCols := range ctx.Columns.WholePreimagesNonSis {
		for _, opened := range roundCols {
			if comp.Columns.Status(opened.GetColID()) != column.Proof {
				utils.Panic(
...
func (ctx *SelfRecursionCtx) SisKey() *ringsis.Key {
	return ctx.VortexCtx.VortexKoalaParams.Key
}
