...
import (
	...
	mimcW "github.com/consensys/accelerated-crypto-monorepo/protocol/dedicated/mimc"
...
func (ctx *SelfRecursionCtx) linearHashAndMerkle() {
	...
	ctx.comp.SubProvers.AppendToInner(roundQ, func(run *wizard.ProverRuntime) {
		...
		for round := 0; round <= totalNumRounds; round++ {
			...
			// nil dereference, it means the rounds was not-committed,
			...
			colSisHash := colSisHashSV.(vortex2.Commitment)

			for i, selectedCol := range q {
		...
		// rounds
		if committedRound != ctx.VortexCtx.NumCommittedRounds() {
			utils.Panic("Committed rounds %v does not match the total number of committed rounds %v", committedRound, ctx.VortexCtx.NumCommittedRounds())
		}
...
func (ctx *SelfRecursionCtx) collapsingPhase() {
	...
	// Currently, only powers of two SIS degree are
	...
	// sanity-check : the size of DhQ collapse must equal
	// sisDeg
	...
	// Create an accessor for collapse^t, where t is
	// the number of opened columns
	collapsePowT := ifaces.Accessor{
		Name: ctx.collapsePowTName(),
		GetVal: func(run ifaces.Runtime) field.Element {
			collapse_ := run.GetRandomCoinField(ctx.Coins.Collapse.Name)
			t := ctx.VortexCtx.NbColsToOpen()
			collapse_.Exp(collapse_, big.NewInt(int64(t)))
			return collapse_
		},
		GetFrontendVariable: func(api frontend.API, run ifaces.GnarkRuntime) frontend.Variable {
			collapse_ := run.GetRandomCoinField(ctx.Coins.Collapse.Name)
			t := ctx.VortexCtx.NbColsToOpen()
			collapse_ = gnarkutil.Exp(api, collapse_, t)
			return collapse_
		},
		Round: round,
	}
