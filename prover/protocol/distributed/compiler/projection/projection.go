	...
	for round := 0; round < numRounds; round++ {
		...
		for _, qName := range queries {
			...
			if bothAAndB {
				...
				p.push(moduleComp, q_, true, true)
			} else if onlyA {
				check(q_.Inp.ColumnA, disc, targetModuleName)
				p.push(moduleComp, q_, true, false)
			} else if onlyB {
				check(q_.Inp.ColumnB, disc, targetModuleName)
				p.push(moduleComp, q_, false, true)
			} else {
				continue
			}
		}
...
func (p *DistributeProjectionCtx) push(comp *wizard.CompiledIOP, q query.Projection, isA, isB bool) {
	var (
		isMultiColumn = len(q.Inp.ColumnA) > 1
		alphaName     = coin.Namef("%v_%v", "MERGING_COIN"+string(q.ID), "FieldFromSeed")
		betaName      = coin.Namef("%v_%v", "EVAL_COIN"+string(q.ID), "FieldFromSeed")
		alpha         coin.Info
		beta          coin.Info
	)
