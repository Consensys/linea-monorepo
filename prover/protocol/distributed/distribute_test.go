...
func TestSerdeDiscoverer(t *testing.T) {
	...
	deserializedDiscVal, err := serialization.DeserializeValue(discSer, serialization.DeclarationMode, reflect.TypeOf(&StandardModuleDiscoverer{}), comp)
	if err != nil {
		t.Fatalf("Failed to deserialize discoverer: %v", err)
	}
	deserializedDisc, ok := deserializedDiscVal.Interface().(*StandardModuleDiscoverer)
	if !ok {
