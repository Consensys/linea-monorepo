...
type Accessor[T zk.Element] interface {
	symbolic.Metadata
	...
	GetFrontendVariable(api zk.APIGen[T], c GnarkRuntime[T]) T
	GetFrontendVariableBase(api zk.APIGen[T], c GnarkRuntime[T]) (T, error)
	GetFrontendVariableExt(api zk.APIGen[T], c GnarkRuntime[T]) gnarkfext.E4Gen[T]
	...
	AsVariable() *symbolic.Expression[T]
}
