...
type Column[T zk.Element] interface {
...
func MustBeInRound[T zk.Element](h Column[T], round int) {
	if round != h.Round() {
...
func AssertNotComposite[T zk.Element](h Column[T]) {
	if h.IsComposite() {
		utils.Panic(
...
func AssertSameLength[T zk.Element](list ...Column[T]) int {
	if len(list) == 0 {
