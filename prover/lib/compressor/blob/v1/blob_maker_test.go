...
func testCompressorSingleSmallBatch(t *testing.T, blocks [][]byte) {
	...
	assert.NoError(t, err)
	_, _, blocksBack, err := DecompressBlob(bm.Bytes(), dict)
	assert.NoError(t, err)
...
func assertBatchesConsistent(t *testing.T, raw, decoded [][]byte) {
	...
	for i := range raw {
		...
		var blockBack []interface{}
		assert.NoError(t, rlp.Decode(bytes.NewReader(decoded[i]), &blockBack))

		var time uint64
		timeBytes := blockBack[0].([]byte)
		timeBytes = append(make([]byte, 8-len(timeBytes), 8), timeBytes...)
		assert.LessOrEqual(t, len(timeBytes), 8, "time should be less than 8 bytes")
		assert.NoError(t, binary.Read(bytes.NewReader(timeBytes), binary.BigEndian, &time))
...
func TestCompressorWithExpandingTx(t *testing.T) {
	...
	// Init bm
	bm, err := NewBlobMaker(blobSizeLimit, testDictPath)
	assert.NoError(err, "init should succeed")
	dict, err := os.ReadFile(testDictPath)
	assert.NoError(err)
...
func TestCompressorWithDecompressorLimit(t *testing.T) {
	...
	assert.Less(int(targetSize), bm.limit)
	...
	rawLen := buf.Len()
	assert.Less(rawLen, int(MaxUncompressedBytes), "block1 should be smaller than MaxBlobPayloadBytes")

	assert.Greater(rawLen*8, int(MaxUncompressedBytes))
...
func init() {
	rootPath, err := getRepoRootPath()
	if err != nil {
		panic(err)
	}

	if testBlocks, err = loadTestBlocks(filepath.Join(rootPath, "testdata/prover-v2/prover-execution/requests")); err != nil {
		panic(err)
	}
...
func makeMaliciousBlock(dict []byte, targetSize int) *types.Block {
	...
	return types.NewBlock(&types.Header{}, []*types.Transaction{maliciousTx}, nil, nil, trie.NewStackTrie(nil))
}
...
func makeFakeBlock(targetSize int) *types.Block {
	...
	signTxFake(&tx)
	return types.NewBlock(&types.Header{}, []*types.Transaction{tx}, nil, nil, trie.NewStackTrie(nil))
}
...
func TestPack(t *testing.T) {
	...
	runTest := func(s1, s2 []byte) {
		// pack them
		buf.Reset()
		written, err := PackAlign(&buf, s1, fr381.Bits-1, WithAdditionalInput(s2))
		assert.NoError(err, "pack should not generate an error")
		assert.Equal(PackAlignSize(len(s1)+len(s2), fr381.Bits-1), int(written), "written bytes should match expected PackAlignSize")
		original, err := UnpackAlign(buf.Bytes(), fr381.Bits-1, false)
