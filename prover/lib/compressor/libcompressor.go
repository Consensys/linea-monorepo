...
//go:generate go build -ldflags "-s -w" -buildmode=c-shared -o libcompressor.so libcompressor.go
func main() {}

var (
	compressor *blob_v0.BlobMaker
	lastError  error
...
func initGo(dataLimit int, dictPath string) bool {
	lock.Lock()
	defer lock.Unlock()
	compressor, lastError = blob_v0.NewBlobMaker(dataLimit, dictPath)
