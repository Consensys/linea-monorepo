...
func TestConstructor(t *testing.T) {
	...
	var (
		...
		one  = NewConstant[zk.NativeElement](1)
	)
	...
	testCases := []struct {
		...
		},
		{
			...
			Explainer:        "LinComb[zk.NativeElement] should be regrouped by associativity",
		},
		{
			Actual:           a.Sub(b),
			ExpectedOperator: LinComb[zk.NativeElement]{Coeffs: []int{1, -1}},
			ExpectedParent:   []*Expression[zk.NativeElement]{b, a},
			Explainer:        "Normally substracting two variables should create a LinComb[zk.NativeElement] object",
		},
		{
			Actual:           a.Sub(zero),
			ExpectedOperator: a.Operator,
		...
		},
		{
			...
			ExpectedOperator: LinComb[zk.NativeElement]{Coeffs: []int{1, 1}},
			ExpectedParent:   []*Expression[zk.NativeElement]{a, NewConstant[zk.NativeElement](-2)},
			Explainer:        "When substracting twice by a constant, this should be simplified into LinComb[zk.NativeElement] with only `1` as coeffs",
		},
		{
