...
contract PlonkVerifierDev {
  ...
  bytes32 immutable CHAIN_CONFIGURATION;
  ...
  function _computeChainConfigurationHash(
    ...
    assembly {
      let len := mload(_data)
      let offset := add(_data, 0x20)

      mimcPayload := mload(0x40)
      let writePtr := add(mimcPayload, 0x20)
      ...
      for {
        ...
        let value := mload(add(offset, mul(i, 0x40)))

        switch iszero(shr(255, value))
        case 1 {
          mstore(writePtr, value)
          writePtr := add(writePtr, 0x20)
        }
        default {
          let most := shr(128, value)
          let least := and(value, 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF)
          mstore(writePtr, most)
          writePtr := add(writePtr, 0x20)
          mstore(writePtr, least)
          writePtr := add(writePtr, 0x20)
        }
      }

      mstore(mimcPayload, sub(writePtr, add(mimcPayload, 0x20)))
