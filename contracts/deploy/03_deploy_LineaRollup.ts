...
import {
  ...
  LINEA_V8_ROLLUP_PAUSE_TYPES_ROLES,
  LINEA_V8_ROLLUP_UNPAUSE_TYPES_ROLES,
  LINEA_V8_ROLLUP_ROLES,
  OPERATOR_ROLE,
...
const func: DeployFunction = async function () {
  ...
  const pauseTypeRoles = getEnvVarOrDefault("LINEA_ROLLUP_PAUSE_TYPE_ROLES", LINEA_V8_ROLLUP_PAUSE_TYPES_ROLES);
  const unpauseTypeRoles = getEnvVarOrDefault("LINEA_ROLLUP_UNPAUSE_TYPE_ROLES", LINEA_V8_ROLLUP_UNPAUSE_TYPES_ROLES);
  const defaultRoleAddresses = generateRoleAssignments(LINEA_V8_ROLLUP_ROLES, lineaRollupSecurityCouncil, [
