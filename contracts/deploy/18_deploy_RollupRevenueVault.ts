...
const func: DeployFunction = withSignerUiSession("18_deploy_RollupRevenueVault.ts", async function () {
  ...
  const securityCouncil = requireAddressOrRegistry(network.name, "L2_SECURITY_COUNCIL", "L2_SECURITY_COUNCIL");
  ...
  const tokenBridge = requireAddressOrRegistry(network.name, "TokenBridge_L2", "ROLLUP_REVENUE_VAULT_TOKEN_BRIDGE");
  const messageService = requireAddressOrRegistry(network.name, "L2MessageService", "L2_MESSAGE_SERVICE_ADDRESS");
