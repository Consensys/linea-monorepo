/**
 * Contract Integrity Verifier - Ethers Adapter Integration Tests
 *
 * Tests the full verification flow using the EthersAdapter with mock RPC responses.
 * Uses real ethers.js for crypto operations, mocks only network calls.
 *
 * Run with: npx ts-node tests/integration.test.ts
 */

import { resolve } from "path";
import { keccak256, toUtf8Bytes, getAddress, AbiCoder, Interface, ZeroAddress } from "ethers";
import {
  loadConfig,
  checkArtifactExists,
  loadArtifact,
...
// ============================================================================
// Mock Ethers Adapter (uses real ethers for crypto, mocks RPC)
// ============================================================================

/**
 * Mock adapter using real ethers.js for crypto operations.
 * Only RPC calls are mocked for offline testing.
 */
class MockEthersAdapter implements Web3Adapter {
  ...
  // Real ethers.js implementations
  ...
  encodeFunctionData(abi: readonly AbiElement[], functionName: string, args?: readonly unknown[]): string {
    ...
    return iface.encodeFunctionData(functionName, args as unknown[]);
  }
...
async function testEthersAdapterCrypto(): Promise<void> {
  console.log("\n=== Ethers Adapter Crypto Tests ===");
...
async function testArtifactLoading(): Promise<void> {
  ...
  assert(selectors.size > 0, "Selectors extracted using ethers");
...
async function testStorageWithEthers(): Promise<void> {
  console.log("\n=== Storage Tests with Ethers ===");
  ...
  // keccak256("linea.storage.YieldManagerStorage") - 1, aligned to 256
  assertContains(
    baseSlot.toLowerCase(),
    "dc1272075efdca0b85fb2d76cbb5f26d954dc18e040d6d0b67071bd5cbd04300",
    "ERC-7201 slot matches expected",
  );
...
async function testVerifierWithEthers(): Promise<void> {
  console.log("\n=== Verifier Tests with Ethers ===");
...
async function testFullIntegration(): Promise<void> {
  ...
  for (const contract of config.contracts) {
    ...
    // Test selector extraction with ethers
...
async function main(): Promise<void> {
  ...
  console.log("Contract Integrity Verifier - Ethers Adapter Integration Tests");
  console.log("=".repeat(60));

  try {
    await testEthersAdapterCrypto();
    await testArtifactLoading();
    await testConfigLoading();
    await testStorageWithEthers();
    await testVerifierWithEthers();
...
// Jest wrapper
describe("Contract Integrity Verifier - Ethers Adapter", () => {
