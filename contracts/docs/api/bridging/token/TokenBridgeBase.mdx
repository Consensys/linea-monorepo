...
```solidity
function __TokenBridge_init(struct ITokenBridge.InitializationData _initializationData) internal virtual
```
...
```solidity
function bridgeToken(address _token, uint256 _amount, address _recipient) public payable virtual
```
...
```solidity
function _bridgeToken(address _token, uint256 _amount, address _recipient) internal virtual
```

This function is the internal implementation of bridgeToken.
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| amount | uint256 | The real amount being transferred to the recipient. |
...
Retrieves the escrow address (default address(this)).

__token is only used in overrides allowing different escrow per token._
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| escrowAddress | address | The address of where the bridged local token will be kept. |

### bridgeTokenWithPermit

```solidity
function bridgeTokenWithPermit(address _token, uint256 _amount, address _recipient, bytes _permitData) external payable virtual
```
...
```solidity
function completeBridging(address _nativeToken, uint256 _amount, address _recipient, uint256 _chainId, bytes _tokenMetadata) external virtual
```

Completes bridging deploying new tokens if needed.
...
```solidity
function _completeBridging(address _nativeToken, uint256 _amount, address _recipient, uint256 _chainId, bytes _tokenMetadata) internal virtual
```
...
```solidity
function _permit(address _token, bytes _permitData) internal virtual
```
