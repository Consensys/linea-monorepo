...
_pauseExpiry applies to all pause types. Pausing with one pause type blocks other pause types from being enacted (unless the SECURITY_COUNCIL_ROLE is used).
...
SECURITY_COUNCIL_ROLE unpause will reset the cooldown._
...
```solidity
function unPauseDueToExpiry(enum IPauseManager.PauseType _pauseType) external
```
