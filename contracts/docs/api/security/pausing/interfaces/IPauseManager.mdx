...
Structure defining a pause type and its associated role.

_This struct is used for both the `_pauseTypeRoles` and `_unPauseTypeRoles` mappings._
...
Enum defining the different types of pausing.

_The pause types are used to pause and unpause specific functionality.
The UNUSED pause type is used as a default value to avoid accidental general pausing.
Enums are uint8 by default._
