...
```solidity
function claimMessage(address _from, address _to, uint256 _fee, uint256 _value, address payable _feeRecipient, bytes _calldata, uint256 _nonce) external virtual
```
...
```solidity
function _claimMessage(address _from, address _to, uint256 _fee, uint256 _value, bytes _calldata, uint256 _nonce) internal virtual
```
