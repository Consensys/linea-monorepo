...
**Generate keys:**
```bash
./gradlew :maru:utils:keytool --args='generateKeys --numberOfKeys=5'
```

**Get Key Info:**
```bash
./gradlew :maru:utils:keytool --args='prefixedKeyInfo --privKey=0x08021220abcd1234...'
```

```bash
./gradlew :maru:utils:keytool --args='secp256k1Info --privKey=0xabcd1234...'
```

**Help:**
```bash
./gradlew :maru:utils:keytool --args='--help'
./gradlew :maru:utils:keytool --args='secp256k1Info --help'
```
...
**Command Line:**
```bash
./gradlew -q :maru:utils:runDifficultyCalculator --args="<currentBlockNumber> <currentTimestamp> <desiredSwitchTime>"
```
...
**Example:**
```bash
./gradlew -q :maru:utils:runDifficultyCalculator --args="1000 1692000000 1692001000"
```
