/**
 * Contract Integrity Verifier - Live Integration Tests (Ethers)
 *
...
async function testRpcConnection(): Promise<void> {
  ...
  assertEqual(Number(network.chainId), CHAIN_IDS.ETHEREUM_SEPOLIA, `Connected to Sepolia (chainId ${CHAIN_IDS.ETHEREUM_SEPOLIA})`);

  const blockNumber = await provider.getBlockNumber();
  assert(blockNumber > 0, `Current block number: ${blockNumber}`);
}

async function testEthersAdapterWithRealRpc(): Promise<void> {
  console.log("\n=== Ethers Adapter with Real RPC ===");

  if (!RPC_URL) {
    skip("Ethers adapter RPC test");
    return;
  }
  ...
  // Test getStorageAt
  const slot0 = await adapter.getStorageAt(proxyAddress, "0x0");
...
async function testVerifierWithRealContract(): Promise<void> {
  ...
  assertContains(implAddress!, SEPOLIA_LINEA_ROLLUP_IMPLEMENTATION.toLowerCase().slice(2), "Correct implementation address");
}
...
async function main(): Promise<void> {
  ...
  console.log("Contract Integrity Verifier - Live Integration Tests (Ethers)");
  console.log("=".repeat(70));
  ...
  try {
    ...
    await testEthersAdapterWithRealRpc();
