...
class RawJsonTracesCounterTest {
  ...
  fun parse_file() {
    ...
    val counted = counter.concreteCountTraces(parsed.get()).component1()?.tracesCounters?.mapKeys { e -> e.key.name }
  ...
  fun parse_string() {
    ...
    assertThat(counted.component1()?.tracesCounters?.mapKeys { e -> e.key.name }).isEqualTo(
      mapOf(
        "ADD" to 0U,
        "BIN" to 0U,
        "BIN_RT" to 0U,
