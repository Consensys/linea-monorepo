...
/**
 * Tests that verify blob transactions are correctly rejected during block import at the protocol
 * level (via LineaBlockTransactionValidatorPlugin).
 *
 ...
 * simultaneously.
 */
class BlobTransactionBlockImportDenialTest : LineaPluginPoSTestBase() {

  override fun getRequestedPlugins(): List<String> = BLOCK_VALIDATOR_REQUESTED_PLUGINS
  ...
  fun blobTransactionsIsRejectedFromNodeImport() {
    ...
    // Assert
    assertBlockImportRejected(response, TransactionTypeValidationErrors.BLOB_TX_NOT_ALLOWED)
  }
  ...
  private fun createBlobTransactionBlockRequest(mapper: ObjectMapper): EngineNewPayloadRequest {
    ...
    // without the LineaTransactionSelectorPlugin and LineaBlockTransactionValidatorPlugin plugins.
