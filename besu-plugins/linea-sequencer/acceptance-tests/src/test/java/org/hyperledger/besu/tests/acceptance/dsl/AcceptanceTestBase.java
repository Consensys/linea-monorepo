...
public abstract class AcceptanceTestBase {
  ...
  private List<Account> createAccountsBulk(int numAccounts, int initialBalanceEther) {
    try {
      ...
      final var senderAccount = accounts.getSecondaryBenefactor();
      ...
      final var transfers = newAccounts.stream().map(
        account -> {
          final var tx =
          ...
          // final var tx2 = accountTransactions
          //   .createTransfer(
          ...
          System.out.println("----TRANSFER_TRANSACTION----");
          ...
          assertThat(decodedTx.getSender().toString()).isEqualTo(senderAccount.getAddress());
          return nodeTxHash;
        }).toList();

      // transfers.forEach(tx -> {
      //   minerNode.verify(eth.expectSuccessfulTransactionReceipt(tx.toHexString()));
      // });
        ...
        () -> {
          final var balance =
            minerNode.nodeRequests().eth()
              .ethGetBalance(address, DefaultBlockParameter.valueOf("LATEST")).send().getBalance();
