...
public class ReferenceTestEnv extends BlockHeader {
  ...
  public ReferenceTestEnv(
      ...
      @JsonProperty("withdrawals") final List<ReferenceTestEnv.EnvWithdrawal> withdrawals,
    ...
    this.withdrawals =
        withdrawals == null
            ? List.of()
            : withdrawals.stream().map(ReferenceTestEnv.EnvWithdrawal::asWithdrawal).toList();
    this.blockHashes =
        blockHashes == null
            ? Map.of()
