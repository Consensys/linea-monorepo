...
(defconstraint address-index (:guard (not-in-padding))
  (begin (* (remains-constant ADDRESS_INDEX)
         ...
         (if-zero (remains-constant ADDRESS_INDEX)
                  (begin (remains-constant SC_ADDRESS_LO)
                         (remains-constant SC_ADDRESS_HI)))))
...
(defconstraint codesize-reached (:guard (not-in-padding))
  (if-zero (remains-constant CODE_FRAGMENT_INDEX)
           (begin (* (remains-constant CODESIZE_REACHED) (- (remains-constant CODESIZE_REACHED) 1))
                  (if-zero (next (- CODESIZE (+ PC 1)))
                           (will-eq CODESIZE_REACHED (+ CODESIZE_REACHED 1))
                           (remains-constant CODESIZE_REACHED)))
...
(defconstraint padding-bit (:guard (not-in-padding))
  (begin
   (if-not-zero (remains-constant CODE_FRAGMENT_INDEX)
