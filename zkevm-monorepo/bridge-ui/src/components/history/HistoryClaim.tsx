...
export default function HistoryClaim({ message, transaction }: Props) {
    ...
    return (
      <div className="flex flex-row items-center space-x-2">
        <button
          ...
          className={classNames('btn btn-primary w-38 rounded-full btn-sm mt-1 no-animation mr-2', {
  ...
  const WaitingForTransaction = ({ loading }: { loading: boolean }) => (
    <div className="flex flex-row items-center space-x-1">
      {loading && <span className="mr-1 loading loading-spinner"></span>}
  ...
  useEffect(() => {
    isWaitingError && toast.error('Funds claiming failed.');
  }, [isWaitingError]);
