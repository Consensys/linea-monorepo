...
import { linea, mainnet, sepolia } from 'viem/chains';
...
const useMessageService = () => {
  ...
  const { messageServiceAddress, fromChain, networkType } = context;
  ...
  useMemo(() => {
    ...
    switch (networkType) {
      case NetworkType.MAINNET:
        _lineaSDK = new LineaSDK({
          l1RpcUrl: `${mainnet.rpcUrls.infura.http.toString()}/${infuraKey}`,
          l2RpcUrl: `${linea.rpcUrls.infura.http.toString()}/${infuraKey}`,
      ...
      case NetworkType.SEPOLIA:
        _lineaSDK = new LineaSDK({
          l1RpcUrl: `${sepolia.rpcUrls.infura.http.toString()}/${infuraKey}`,
          l2RpcUrl: `${lineaSepolia.rpcUrls.infura.http.toString()}/${infuraKey}`,
          network: 'localhost',
          mode: 'read-only',
        });
        break;
    ...
    const newLineaSDKContracts: LineaSDKContracts = {
      L1: _lineaSDK.getL1Contract(config.networks[networkType].L1.messageServiceAddress),
      L2: _lineaSDK.getL2Contract(config.networks[networkType].L2.messageServiceAddress),
    };
  ...
  const getMessagesStatusesByTransactionHash = useCallback(
    ...
    [lineaSDKContracts, getMessagesByTransactionHash, lineaSDK]
  );
      ...
      if (address && txNetworkLayer && txNetworkType) {
        try {
          ...
          if (!proof) {
            ...
            writeConfig = await prepareWriteContract({
              address: messageServiceAddress,
              abi: MessageService.abi,
