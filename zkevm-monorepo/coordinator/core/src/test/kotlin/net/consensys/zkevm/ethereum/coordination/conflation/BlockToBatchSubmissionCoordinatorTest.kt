...
class BlockToBatchSubmissionCoordinatorTest {
  companion object {
    ...
    private val tracesCounters = emptyTracesCounts()
    private val l1DataSize = 0U
  }
  ...
  private fun createBlockToBatchSubmissionCoordinator(
    ...
    val tracesCountersClient: TracesCountersClientV1 = mock<TracesCountersClientV1?>().also {
      whenever(
    ...
    val tracesFileManager: TracesWatcher = mock<TracesWatcher?>().also {
      whenever(
        it.waitRawTracesGenerationOf(
          randomExecutionPayload.blockNumber,
          randomExecutionPayload.blockHash
        )
      ).thenReturn(
        SafeFuture.completedFuture("")
