...
import net.consensys.zkevm.coordinator.clients.prover.ExecutionProofFileNameProvider
...
class CoordinatorApp(private val configs: CoordinatorConfig) {
  ...
  private fun createExecutionProverClient(config: ProverConfig): FileBasedExecutionProverClient {
    val fileNameProvider: ProverFilesNameProvider = ExecutionProofFileNameProvider(
    ...
    return FileBasedExecutionProverClient(
      ...
      fileNamesProvider = fileNameProvider
    )
  }
    ...
    BlobsRepositoryImpl(
      BlobsPostgresDao(
        BlobsPostgresDao.Config(
          configs.blobSubmission.maxBlobsToReturn.toUInt()
        ),
        sqlClient
      )
    )

  private val aggregationsRepository = AggregationsRepositoryImpl(PostgresAggregationsDao(sqlClient))
  ...
  private val l1FeeHistoriesRepository =
    FeeHistoriesRepositoryImpl(
      FeeHistoriesRepositoryImpl.Config(configs.l1DynamicGasPriceCapService.feeHistoryFetcher.rewardPercentiles),
  ...
  private val requestFileCleanup = DirectoryCleaner(
    ...
    fileFilter = DirectoryCleaner.JSON_FILE_FILTER
  )
