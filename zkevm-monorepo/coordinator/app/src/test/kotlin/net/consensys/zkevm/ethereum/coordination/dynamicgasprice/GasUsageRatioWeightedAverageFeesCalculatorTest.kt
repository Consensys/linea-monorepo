package net.consensys.zkevm.ethereum.coordination.dynamicgasprice

import net.consensys.linea.FeeHistory
import org.assertj.core.api.Assertions
import org.junit.jupiter.api.Test
import java.math.BigDecimal
import java.math.BigInteger

class GasUsageRatioWeightedAverageFeesCalculatorTest {
  val feesCalculator =
    GasUsageRatioWeightedAverageFeesCalculator(
      GasUsageRatioWeightedAverageFeesCalculator.Config(
        baseFeeCoefficient = BigDecimal("0.1"),
        priorityFeeCoefficient = BigDecimal("1.2"),
        baseFeeBlobCoefficient = BigDecimal("0.1"),
        blobSubmissionExpectedExecutionGas = BigDecimal(120_000.0),
        expectedBlobGas = BigDecimal(131_000.0)
      )
    )

  @Test
  fun calculateFees_singleBlockHistory() {
    val feeHistory = FeeHistory(
      oldestBlock = BigInteger.valueOf(100),
      baseFeePerGas = listOf(100, 110).map { it.toBigInteger() },
      reward = listOf(1000).map { listOf(it.toBigInteger()) },
      gasUsedRatio = listOf(0.25.toBigDecimal()),
      baseFeePerBlobGas = listOf(100, 110).map { it.toBigInteger() },
      blobGasUsedRatio = listOf(0.16.toBigDecimal())
    )
    // (100*0.1 + 1000*1.2)*0.25/0.25 = 1210
    // ((100*0.1)*0.16)/0.16 = 1210
    val calculatedL2GasPrice = feesCalculator.calculateFees(feeHistory)
    Assertions.assertThat(calculatedL2GasPrice).isEqualTo(BigInteger.valueOf(1220))
  }

  @Test
  fun calculateFees_singleBlockHistory_zeroUsageRatio() {
    val feeHistory = FeeHistory(
      oldestBlock = BigInteger.valueOf(100),
      baseFeePerGas = listOf(100, 110).map { it.toBigInteger() },
      reward = listOf(1000).map { listOf(it.toBigInteger()) },
      gasUsedRatio = listOf(0.0.toBigDecimal()),
      baseFeePerBlobGas = listOf(100, 110).map { it.toBigInteger() },
      blobGasUsedRatio = listOf(0.0.toBigDecimal())
    )
    // (100*0.1 + 1000*1.2) = 1210
    // (((100*0.1) * 0.5) / 0.5 ) * (131000.0 / 120000.0) = 10.9166666
    val calculatedL2GasPrice = feesCalculator.calculateFees(feeHistory)
    Assertions.assertThat(calculatedL2GasPrice).isEqualTo(BigInteger.valueOf(1220))
  }

  @Test
  fun calculateFees_MultipleBlockHistory() {
    val feeHistory = FeeHistory(
      oldestBlock = BigInteger.valueOf(100),
      baseFeePerGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      reward = listOf(1000, 1100, 1200, 1300).map { listOf(it.toBigInteger()) },
      gasUsedRatio = listOf(0.25.toBigDecimal(), 0.5.toBigDecimal(), 0.75.toBigDecimal(), 0.9.toBigDecimal()),
      baseFeePerBlobGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      blobGasUsedRatio = listOf(0.16.toBigDecimal(), 0.5.toBigDecimal(), 0.66.toBigDecimal(), 0.83.toBigDecimal())
    )

    // Weighted Average:
    // ((100*0.1 + 1000*1.2)*0.25 + (110*0.1 + 1100*1.2)*0.50 + (120*0.1 + 1200*1.2)*0.75 + (130*0.1 + 1300*1.2)*0.9)/(0.25+0.5+0.75+0.9) = 1446.95833333
    // (((100*0.1)*0.16 + (110*0.1)*0.50 + (120*0.1)*0.66 + (130*0.1)*0.83)/(0.16+0.5+0.66+0.83)) * (131000.0 / 120000.0) = 12.0046511
    val calculatedL2GasPrice = feesCalculator.calculateFees(feeHistory)
    Assertions.assertThat(calculatedL2GasPrice).isEqualTo(BigInteger.valueOf(1459))
  }

  @Test
  fun calculateFees_equalUsageRatio_shouldEqualSimpleAverage() {
    val feeHistory = FeeHistory(
      oldestBlock = BigInteger.valueOf(100),
      baseFeePerGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      reward = listOf(1000, 1100, 1200, 1300).map { listOf(it.toBigInteger()) },
      gasUsedRatio = listOf(0.5.toBigDecimal(), 0.5.toBigDecimal(), 0.5.toBigDecimal(), 0.5.toBigDecimal()),
      baseFeePerBlobGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      blobGasUsedRatio = listOf(0.5.toBigDecimal(), 0.5.toBigDecimal(), 0.5.toBigDecimal(), 0.5.toBigDecimal())
    )

    // Simple Average:
    // ((100*0.1 + 1000*1.2)*0.5 + (110*0.1 + 1100*1.2)*0.5 + (120*0.1 + 1200*1.2)*0.5 + (130*0.1 + 1300*1.2)*0.5)/(0.5+0.5+0.5+0.5) = 1391.5
    // (((100*0.1)*0.5 + (110*0.1)*0.5 + (120*0.1)*0.5 + (130*0.1)*0.5)/(0.5+0.5+0.5+0.5)) * (131000.0 / 120000.0) = 12.55416
    val calculatedL2GasPrice = feesCalculator.calculateFees(feeHistory)
    Assertions.assertThat(calculatedL2GasPrice).isEqualTo(BigInteger.valueOf(1404))
  }

  @Test
  fun calculateFees_zeroUsage_emptyL1_blocks_should_return_SimpleAverage() {
    val feeHistory = FeeHistory(
      oldestBlock = BigInteger.valueOf(100),
      baseFeePerGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      reward = listOf(1000, 1100, 1200, 1300).map { listOf(it.toBigInteger()) },
      gasUsedRatio = listOf(0.0.toBigDecimal(), 0.0.toBigDecimal(), 0.0.toBigDecimal(), 0.0.toBigDecimal()),
      baseFeePerBlobGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      blobGasUsedRatio = listOf(0.0.toBigDecimal(), 0.0.toBigDecimal(), 0.0.toBigDecimal(), 0.0.toBigDecimal())
    )

    // Simple Average:
    // ((100*0.1 + 1000*1.2)*1 + (110*0.1 + 1100*1.2)*1 + (120*0.1 + 1200*1.2)*1 + (130*0.1 + 1300*1.2)*1)/4) = 1391.5
    // (((100*0.1)*0.5 + (110*0.1)*0.5 + (120*0.1)*0.5 + (130*0.1)*0.5)/(0.5+0.5+0.5+0.5)) * (131000.0 / 120000.0) = 12.55416
    val calculatedL2GasPrice = feesCalculator.calculateFees(feeHistory)
    Assertions.assertThat(calculatedL2GasPrice).isEqualTo(BigInteger.valueOf(1404))
  }

  @Test
  fun calculateFees_zeroUsage_emptyL1Blobs_should_return_SimpleAverage() {
    val feeHistory = FeeHistory(
      oldestBlock = BigInteger.valueOf(100),
      baseFeePerGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      reward = listOf(1000, 1100, 1200, 1300).map { listOf(it.toBigInteger()) },
      gasUsedRatio = listOf(0.0.toBigDecimal(), 0.0.toBigDecimal(), 0.0.toBigDecimal(), 0.0.toBigDecimal()),
      baseFeePerBlobGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      blobGasUsedRatio = listOf(0.0.toBigDecimal(), 0.0.toBigDecimal(), 0.0.toBigDecimal(), 0.0.toBigDecimal())
    )

    // Simple Average:
    // ((100*0.1 + 1000*1.2)*1 + (110*0.1 + 1100*1.2)*1 + (120*0.1 + 1200*1.2)*1 + (130*0.1 + 1300*1.2)*1)/4) = 1391.5
    // (((100*0.1)*0.5 + (110*0.1)*0.5 + (120*0.1)*0.5 + (130*0.1)*0.5)/(0.5+0.5+0.5+0.5)) * (131000.0 / 120000.0) = 12.55416
    val calculatedL2GasPrice = feesCalculator.calculateFees(feeHistory)
    Assertions.assertThat(calculatedL2GasPrice).isEqualTo(BigInteger.valueOf(1404))
  }

  @Test
  fun calculateFees_MultipleBlockHistory_emptyBlobLists_should_return_value() {
    val feeHistory = FeeHistory(
      oldestBlock = BigInteger.valueOf(100),
      baseFeePerGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      reward = listOf(1000, 1100, 1200, 1300).map { listOf(it.toBigInteger()) },
      gasUsedRatio = listOf(0.25.toBigDecimal(), 0.5.toBigDecimal(), 0.75.toBigDecimal(), 0.9.toBigDecimal()),
      baseFeePerBlobGas = emptyList(),
      blobGasUsedRatio = emptyList()
    )

    // Weighted Average:
    // ((100*0.1 + 1000*1.2)*0.25 + (110*0.1 + 1100*1.2)*0.50 + (120*0.1 + 1200*1.2)*0.75 + (130*0.1 + 1300*1.2)*0.9)/(0.25+0.5+0.75+0.9) = 1446.95833333
    val calculatedL2GasPrice = feesCalculator.calculateFees(feeHistory)
    Assertions.assertThat(calculatedL2GasPrice).isEqualTo(BigInteger.valueOf(1447))
  }

  @Test
  fun calculateFees_partialZeroUsage_emptyL1_blocks_should_return_SimpleAverage() {
    // Very unlikely scenario in L1 network with activity.
    // Only happen for local dev or test networks, so we don't need to optimize for it.
    val feeHistory = FeeHistory(
      oldestBlock = BigInteger.valueOf(100),
      baseFeePerGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      reward = listOf(1000, 1100, 1200, 1300).map { listOf(it.toBigInteger()) },
      gasUsedRatio = listOf(0.75.toBigDecimal(), 0.0.toBigDecimal(), 0.75.toBigDecimal(), 0.0.toBigDecimal()),
      baseFeePerBlobGas = listOf(100, 110, 120, 130, 140).map { it.toBigInteger() },
      blobGasUsedRatio = listOf(0.83.toBigDecimal(), 0.0.toBigDecimal(), 0.83.toBigDecimal(), 0.0.toBigDecimal())
    )

    // Weighted Average:
    // ((100*0.1 + 1000*1.2)*0.75 + (110*0.1 + 1100*1.2)*0.0 + (120*0.1 + 1200*1.2)*0.75 + (130*0.1 + 1300*1.2)*0.0)/(0.75+0.0+0.75+0.0) = 1331
    // (((100*0.1)*0.0 + (110*0.1)*0.83 + (120*0.1)*0.0 + (130*0.1)*0.83)/(0.83+0.0+0.83+0.0)) * (131000.0 / 120000.0) = 13.1
    val calculatedL2GasPrice = feesCalculator.calculateFees(feeHistory)
    Assertions.assertThat(calculatedL2GasPrice).isEqualTo(BigInteger.valueOf(1342))
  }
}
