...
class FeeHistoriesPostgresDao(
  ...
  override fun saveNewFeeHistory(feeHistory: FeeHistory, rewardPercentiles: List<Double>): SafeFuture<Unit> {
    val futures: List<SafeFuture<*>> = feeHistory.reward.mapIndexed { i, reward ->
      val params = listOf(
        clock.now().toEpochMilliseconds(),
        feeHistory.oldestBlock.toLong() + i,
        feeHistory.baseFeePerGas[i].toLong(),
        feeHistory.baseFeePerBlobGas.getOrElse(i) { BigInteger.ZERO }.toLong(),
        feeHistory.gasUsedRatio[i].toFloat(),
        feeHistory.blobGasUsedRatio.getOrElse(i) { BigDecimal.ZERO }.toFloat(),
        reward.map { it.toLong() }.toTypedArray(),
        rewardPercentiles.map { it.toFloat() }.toTypedArray()
      )
      queryLog.log(Level.TRACE, upsertSql, params)
      upsertQuery.execute(Tuple.tuple(params))
        .map { }
        .recover { th ->
    ...
    return SafeFuture.allOf(futures.stream()).thenApply {}
  }
  ...
  override fun findBaseFeePerGasAtPercentile(
    ...
    return getNthPercentileOfBaseFeePerGasQuery
      ...
      .thenApply { rowSet ->
        if (rowSet.size() > 0) {
          rowSet.first().getBigDecimal("percentile_value")?.run {
            this.toBigInteger()
          }
        } else {
          null
  ...
  override fun findAverageRewardAtPercentile(
    ...
    return getAvgNthPercentileRewardQuery
      ...
      .thenApply { rowSet ->
        if (rowSet.size() > 0) {
          rowSet.first().getBigDecimal("avg_reward")?.run {
            this.toBigInteger()
          }
        } else {
          null
