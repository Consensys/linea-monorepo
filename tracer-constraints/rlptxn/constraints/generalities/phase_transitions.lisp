...
(defun (upcoming-phase-transition-type-3)
    (force-bin (+
        (* IS_RLP_PREFIX                (next IS_CHAIN_ID                ))
        (* IS_CHAIN_ID                  (next IS_NONCE                   ))
        (* IS_NONCE                     (next IS_MAX_PRIORITY_FEE_PER_GAS))
        ...
        (* IS_VALUE                     (next IS_DATA                    ))
        (* IS_DATA                      (next IS_ACCESS_LIST             ))
        (* IS_ACCESS_LIST               (next IS_MAX_FEE_PER_BLOB_GAS    ))
        (* IS_MAX_FEE_PER_BLOB_GAS      (next IS_BLOB_HASH               ))
        (* IS_BLOB_HASH                 (next IS_Y                       ))
        ;; (* IS_BETA                      (next IS_BETA                 ))
        (* IS_Y                         (next IS_R                       ))
        (* IS_R                         (next IS_S                       ))
        (* IS_S                         (next IS_RLP_PREFIX              ))
    ...
    (defun (will-remain-in-same-phase-type-3)
        (force-bin (+
            (* IS_RLP_PREFIX                (next IS_RLP_PREFIX              ))
            (* IS_CHAIN_ID                  (next IS_CHAIN_ID                ))
            (* IS_NONCE                     (next IS_NONCE                   ))
            ...
            (* IS_DATA                      (next IS_DATA                    ))
            (* IS_ACCESS_LIST               (next IS_ACCESS_LIST             ))
            (* IS_MAX_FEE_PER_BLOB_GAS      (next IS_MAX_FEE_PER_BLOB_GAS    ))
            (* IS_BLOB_HASH                 (next IS_BLOB_HASH               ))
            ;; (* IS_BETA                      (next IS_BETA                    ))
            (* IS_Y                         (next IS_Y                       ))
            (* IS_R                         (next IS_R                       ))
            (* IS_S                         (next IS_S                       ))
...
(defun (upcoming-phase-transition)
        (force-bin (+ (* (upcoming-phase-transition-type-0) TYPE_0)
                      ...
                      (* (upcoming-phase-transition-type-3) TYPE_2)
                      (* (upcoming-phase-transition-type-3) TYPE_3))))
...
(defconstraint type-3-phases ()
    (if-not-zero TYPE_3 (eq! (+ (upcoming-phase-transition-type-3) (will-remain-in-same-phase-type-3)) 1)))
