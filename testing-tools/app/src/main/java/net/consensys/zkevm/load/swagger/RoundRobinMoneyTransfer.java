...
/**
 * The test will create nbWallets new wallets and make each of them send nbTransfers requests to
 * each other in a round robin fashion.
 */
public class RoundRobinMoneyTransfer extends Scenario {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!RoundRobinMoneyTransfer.openapiRequiredFields
          .isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in RoundRobinMoneyTransfer is not found in the empty JSON string",
                RoundRobinMoneyTransfer.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!RoundRobinMoneyTransfer.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `RoundRobinMoneyTransfer` properties. JSON: %s",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      return (TypeAdapter<T>)
          new TypeAdapter<RoundRobinMoneyTransfer>() {
            @Override
