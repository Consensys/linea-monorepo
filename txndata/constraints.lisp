...
(defconstraint    comparison---nonce-must-not-exceed-EIP-2681-max-nonce    (:guard (first-row-of-new-transaction))
                  (begin
                    (small-call-to-LT    comparison---nonce-row-offset NONCE EIP2681_MAX_NONCE)
...
(defconstraint    comparison---initial-balance-must-cover-value-plus-maximal-gas-cost    (:guard (first-row-of-new-transaction))
                  (begin
                    (small-call-to-LEQ    comparison---initial-balance-row-offset INITIAL_BALANCE (+ (value) (* (max_fee) (gas_limit))))
                    (result-must-be-true  comparison---initial-balance-row-offset)))
...
(defconstraint    comparison---gas-limit-must-cover-upfront-gas-cost    (:guard (first-row-of-new-transaction))
                  (begin
                    (small-call-to-LEQ    comparison---sufficient-gas-row-offset (upfront_gas_cost) (gas_limit))
                    (result-must-be-true  comparison---sufficient-gas-row-offset)))

(defconstraint    integer-division---compute-upper-limit-for-refunds   (:guard (first-row-of-new-transaction))
                  (begin
                    (call-to-EUC    comparison---upper-limit-refunds-row-offset (execution_gas_cost) MAX_REFUND_QUOTIENT)))

(defun (execution_gas_cost) (- (gas_limit) GAS_LEFTOVER))
(defun (refund_limit)       (shift RES comparison---upper-limit-refunds-row-offset))
...
(defun (get_full_refund) (force-bool (shift RES comparison---effective-refund-row-offset)))
...
(defun (nonzero-data-size) (force-bool (- 1 (shift RES comparison---detecting-empty-call-data-row-offset))))

(defconstraint    comparison---comparing-the-maximum-gas-price-against-the-basefee    (:guard (first-row-of-new-transaction))
                  (begin
                    (small-call-to-LEQ    comparison---max-fee-and-basefee-row-offset BASEFEE (maximal_gas_price))
                    (result-must-be-true  comparison---max-fee-and-basefee-row-offset)))
...
(defconstraint    comparison-for-type-2---comparing-max-fee-and-max-priority-fee    (:guard    (*   (first-row-of-new-transaction)   TYPE2))
                  (begin (small-call-to-LEQ    comparison---maxfee-and-max-priority-fee-row-offset (max_priority_fee) (max_fee))
                         (result-must-be-true  comparison---maxfee-and-max-priority-fee-row-offset)))

(defconstraint    comparison-for-type-2---computing-the-effective-gas-price         (:guard    (*   (first-row-of-new-transaction)   TYPE2))
                  (small-call-to-LEQ   comparison---computing-effective-gas-price-row-offset (+ (max_priority_fee) BASEFEE) (max_fee)))

(defun (get_full_tip) (force-bool (shift RES comparison---computing-effective-gas-price-row-offset)))
...
(defconstraint setting-gas-init (:guard (first-row-of-new-transaction))
               (if-not-zero TYPE0
...
(defconstraint partially-setting-requires-evm (:guard (first-row-of-new-transaction))
               (if-not-zero IS_DEP
