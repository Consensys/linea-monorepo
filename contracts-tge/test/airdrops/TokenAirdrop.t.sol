...
contract TokenAirdropTest is Test {
  ...
  function setUp() public {
    ...
    airdrop = new TokenAirdrop(address(token), owner, claimEnd, address(primaryFactor), address(primaryConditional), address(secondaryFactor));
  ...
  function test_Succeeds_With_ConstructorEmittingAirdropConfigured() public {
    ...
    emit TokenAirdrop.AirdropConfigured(address(token), address(primaryFactor), address(primaryConditional), address(secondaryFactor), claimEnd);

    // Deploy the airdrop contract using both primary and secondary factors.
    TokenAirdrop localAirdrop = new TokenAirdrop(address(token), owner, claimEnd, address(primaryFactor), address(primaryConditional), address(secondaryFactor));
  ...
  function test_Reverts_If_ConstructorHasZeroToken() public {
    ...
    new TokenAirdrop(address(0), owner, claimEnd, address(primaryFactor), address(primaryConditional), address(secondaryFactor));
  }
  ...
  function test_Reverts_If_ConstructorHasClaimEndTooShort() public {
    ...
    new TokenAirdrop(address(token), owner, block.timestamp, address(primaryFactor), address(primaryConditional), address(secondaryFactor));
  }

  function test_Reverts_If_ConstructorHasZeroAddess() public {
    vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableInvalidOwner.selector, address(0)));
    new TokenAirdrop(address(token), address(0), claimEnd, address(primaryFactor), address(primaryConditional), address(secondaryFactor));
  }

  function test_Reverts_If_OnlyPrimaryAddressHasZeroAddess() public {
    vm.expectRevert(abi.encodeWithSelector(TokenAirdrop.ZeroAddressNotAllowed.selector));
    new TokenAirdrop(address(token), owner, claimEnd, address(0), address(primaryConditional), address(secondaryFactor));
  }
  ...
  function test_Succeeds_With_PrimaryOnlyUser1Claiming_AllSecondaryValuesZero() public {
    TokenAirdrop onlyPrimarAirdrop = new TokenAirdrop(address(token), owner, claimEnd, address(primaryFactor), address(primaryConditional), address(0));
  ...
  function test_Succeeds_With_SecondaryOnlyUser1Claiming_AllPrimaryValuesZeros() public {
    TokenAirdrop onlySecondaryAirdrop = new TokenAirdrop(address(token), owner, claimEnd, address(0), address(0), address(secondaryFactor));
